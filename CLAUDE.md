...
**bdo (Bushido)** is a high-performance CLI proxy that minimizes LLM token consumption by filtering and compressing command outputs. It achieves 60-90% token savings on common development operations through smart filtering, grouping, truncation, and deduplication.

It is a fork of [rtk (Rust Token Killer)](https://github.com/rtk-ai/rtk) with the binary renamed to `bdo` (crate `bushido`), plus fixes for git argument parsing and modern JavaScript stack support (pnpm, vitest, Next.js, TypeScript, Playwright, Prisma).

## Development Commands

> **Note**: If bdo is installed, prefer `bdo <cmd>` over raw commands for token-optimized output.
> All commands work with passthrough support even for subcommands bdo doesn't specifically handle.

### Build & Run
```bash
cargo build                   # raw
bdo cargo build               # preferred (token-optimized)
...
bdo cargo test                # preferred (token-optimized)
...
bdo cargo clippy --all-targets # preferred
```
...
bdo uses a **command proxy architecture**: `main.rs` routes CLI commands via a Clap `Commands` enum to specialized filter modules in `src/cmds/*/`, each of which executes the underlying command and compresses its output. Token savings are tracked in SQLite via `src/core/tracking.rs`.
...
**Usage**: `bdo proxy <command> [args...]`

**Benefits**:
- **Bypass Bushido filtering**: Workaround bugs or get full unfiltered output
- **Track usage metrics**: Measure which commands Claude uses most (visible in `bdo gain --history`)
- **Guaranteed compatibility**: Always works even if Bushido doesn't implement the command

**Examples**:
```bash
bdo proxy git log --oneline -20    # Full git log output (no truncation)
bdo proxy npm install express      # Raw npm output (no filtering)
bdo proxy curl https://api.example.com/data  # Any command works
```

All proxy commands appear in `bdo gain --history` with 0% savings (input = output).
...
```bash
hyperfine 'bdo git log -10' --warmup 3          # before
cargo build --release
hyperfine 'target/release/bdo git log -10' --warmup 3  # after (should be <10ms)
```
...
```bash
pwd  # Verify you're in the bdo project root
