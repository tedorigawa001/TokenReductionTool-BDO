...
Four principles guide every Bushido design decision. Understanding them helps you write contributions that fit naturally into the project.
...
> Example: `bdo cargo test` shows failures only (90% savings). But `bdo cargo test -- --nocapture` preserves all output because the user explicitly asked for it.
...
If a filter fails, fall back to raw output. Bushido should never prevent a command from executing or producing output. Better to pass through unfiltered than to error out. Same for hooks: exit 0 on all error paths so the agent's command runs unmodified.
...
<10ms startup. No async runtime. No config file I/O on the critical path. If developers perceive any delay, they'll disable Bushido. Speed is the difference between adoption and abandonment.
...
- Others features not related to a LLM-proxy like Bushido
...
Bushido uses [Conventional Commits](https://www.conventionalcommits.org/) and [release-please](https://github.com/googleapis/release-please) to **auto-generate CHANGELOG.md, version bumps, and GitHub releases**. Never edit `CHANGELOG.md` manually — it is fully managed by release-please from your commit messages.
...
- [ ] Manual test: run `bdo <cmd>` and inspect output
...
This protects Bushido's shell execution capabilities against injection attacks and supply chain vulnerabilities.

---

**Thank you for contributing to bdo!**
