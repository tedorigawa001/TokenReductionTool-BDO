...
Bushido collects anonymous, aggregate usage metrics once per day to help improve the product. Telemetry is **disabled by default** and requires explicit consent during `bdo init` or `bdo telemetry enable`.

## Data Collector

**Entity**: `Bushido AI Labs`
**Contact**: https://github.com/tedorigawa001/TokenReductionTool/issues

## Why we collect telemetry

Bushido supports 100+ commands across 15+ ecosystems. Without telemetry, we have no way to know:
...
- How much value Bushido delivers to users (token savings in $ terms)
- Whether users stay engaged over time or churn after trying Bushido
...
1. **Once per day** (23-hour interval), Bushido sends a single HTTPS POST to our telemetry endpoint
...
| `low_savings_commands` | `["bdo docker ps:25%"]` | Commands averaging <30% savings — filters to improve |
...
| Field | Example | Purpose |
|-------|---------|---------|
| `has_config_toml` | `true` | Whether user has customized Bushido config |
...
| Field | Example | Purpose |
|-------|---------|---------|
| `meta_usage` | `{"gain": 5, "discover": 2}` | Which Bushido features are actually used |
...
Telemetry requires explicit opt-in consent (GDPR Art. 6, 7). Consent is requested during `bdo init` or via `bdo telemetry enable`. Without consent, no data is sent.

```bash
bdo telemetry status     # Check current consent state
bdo telemetry enable     # Give consent (interactive prompt)
bdo telemetry disable    # Withdraw consent
bdo telemetry forget     # Withdraw consent + delete local data + request server erasure
```
...
- **Client-side**: the local SQLite database (`~/.local/share/rtk/tracking.db`) retains data for **90 days** by default (configurable via `tracking.history_days` in `config.toml`). Deleted entirely by `bdo telemetry forget`.

## Your Rights (GDPR)

Under the EU General Data Protection Regulation, you have the right to:

- **Access** your data: `bdo telemetry status` shows your device hash; the telemetry payload is fully documented above.
...
1. Run `bdo telemetry forget` — this disables telemetry, deletes your device salt, ping marker, and local tracking database (`history.db`), then sends an erasure request to the server.
2. If the server is unreachable, the CLI prints your full device hash and fallback instructions to open an issue at https://github.com/tedorigawa001/TokenReductionTool/issues for manual erasure.
3. You can also open an issue at https://github.com/tedorigawa001/TokenReductionTool/issues directly to request manual erasure.

## Data Handling

- Telemetry endpoint URL and auth token are injected at **compile time** via `option_env!()` — they are not in the source code
- All communications use HTTPS (TLS)
- Data is used exclusively for Bushido product improvement
- No data is sold or shared with third parties
- Aggregate statistics may be published (e.g. "70% of Bushido users are on macOS")
...
- **No async**: consistent with Bushido's single-threaded design
- **Compile-time gating**: if `BDO_TELEMETRY_URL` is not set at build time, all telemetry code is dead — the binary makes zero network calls
