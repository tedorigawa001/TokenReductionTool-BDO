...
- ✅ ~~**`cat f | shasum` 等パイプ時の raw passthrough**~~（2026-09-20 設計判断・実装）:
  **「パイプに流すセグメントは hook が書き換えない」で決着。**
  - **判断の核**: エージェントが見るのはパイプの**最終出力だけ**。左側を `bdo` に
    書き換えても節約は 0 トークンで、下流プログラムが受け取るデータだけが変わる。
    実測: `cat src/main.rs | wc -l` が 3614 → **400**、`git log --oneline | wc -l` が
    105 → **50**。「フィルタ出力は本物の短縮版と見分けがつかないこと」（CONTRIBUTING）
    に反し、しかも得るものが無い。
  - 「非 TTY なら raw」（stdout の isatty 判定）は不採用 — エージェント経由では
    stdout が常にパイプなので全フィルタが死ぬ。パイプの有無を知っているのは hook
    だけなので、判断は hook 側で行う。
  - コードには `find`/`fd` だけの例外（#439）が既にあり、問題が部分的に認識されて
    いた。それを全コマンドに一般化しただけ（`registry.rs` の `rewrite_compound`
    Pipe 分岐）。`a && b | c` では `a` は書き換え、`b` は据え置き。
  - 既存テスト10件の期待値を反転。`tests/pipe_passthrough.rs`（3件、実バイナリ）で
    「hook 通過後のコマンドを実行しても元と同じ結果」という不変条件を固定。
  - **副産物**: `cargo test` をローカルで通す過程で `test --changed` の欠陥を発見 —
    ランナー不在（`go` 未インストール）が Err で伝播しループ全体を中断していた。
    他言語のテストまで走らなくなるので修正（`ci.rs`）。CI ランナーには go が
    プリインストールされているため CI では見えなかった。
  - **将来の上位案（未着手）**: 書き換えを止める代わりに `bdo proxy` や統一 raw
    フラグ（バックログ「raw バイパスの統一」）で包めば、素通し+tracking を両立
    できる。パイプ左側は今のところ `bdo gain` に載らない。
...
- **リリース成果物は完全性のみで真正性がない**（2026-09-24 記録）: `install.sh` は
  `.sha256` サイドカーを検証するが、サイドカーはバイナリと**同じリリースから**取得
  する。転送中の破損・途中切れは検出できるが、リリース自体（またはリポジトリの
  書き込み権限）が侵害された場合は両方差し替えられるので防げない。Homebrew / npm も
  同じリリースを参照するため同条件。強化策は cargo-dist の `github-attestations`
  （リリース成果物に GitHub 署名付きの build provenance を付け、利用者は
  `gh attestation verify` で「このリポジトリの CI がこのコミットから作った」ことを
  検証できる）。`release.yml` に `id-token: write` / `attestations: write` 権限の
  追加が必要で、`allow-dirty = ["ci"]` 下の手動 hardening と両立させる作業になる。
  利用者側の検証手順も伴うため、配布先が増えるか外部からの利用が増えたら着手。
