# bdo Architecture Documentation

> **Deep reference** for Bushido's system design, filtering taxonomy, performance characteristics, and architecture decisions. For a guided tour of the end-to-end flow, start with [TECHNICAL.md](TECHNICAL.md).

**bdo (Rust Token Killer)** is a high-performance CLI proxy that minimizes LLM token consumption through intelligent output filtering and compression.
...
Command                 Raw Time    bdo Time    Overhead    Savings
─────────────────────────────────────────────────────────────────────────
...
├─────────┼──────────────────────────────────────────────────────┤
│ 0       │ Success                                              │
│ 1       │ bdo internal error (parsing, filtering, etc.)        │
...
Check existing CLAUDE.md:
  ...
  • else      → ./CLAUDE.md
      ...
      └─ Not exists? → Continue
      ↓
Prompt: "Initialize bdo for LLM usage? [y/N]"
      ↓ Yes
Write template:
┌─────────────────────────────────────┐
│ # CLAUDE.md                         │
│                                     │
│ Use `bdo` prefix for commands:      │
│ - bdo git status                    │
│ - bdo lint                          │
│ - bdo test                          │
│                                     │
...
└─────────────────────────────────────┘
      ↓
Success: "✓ Initialized bdo for LLM integration"
```
...
┌──────────────────────┬──────────────┬──────────────┐
│ Operation            │ bdo Overhead │ Total Time   │
├──────────────────────┼──────────────┼──────────────┤
│ bdo git status       │ +8ms         │ 58ms         │
│ bdo grep "pattern"   │ +12ms        │ 145ms        │
│ bdo read file.rs     │ +5ms         │ 15ms         │
│ bdo lint             │ +15ms        │ 2.5s         │
└──────────────────────┴──────────────┴──────────────┘
...
| **Proxy Pattern** | bdo sits between user and tool, transforming output |
