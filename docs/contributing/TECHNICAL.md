# Bushido Technical Documentation

> **Start here** for a guided tour of how Bushido works end-to-end.
>
...
Bushido sits between the agent and the CLI, filtering outputs to keep only what matters. This achieves 60-90% token savings per command, reducing costs and increasing effective context window utilization. Bushido is a single Rust binary with no runtime dependencies beyond the compiled binary itself, adding less than 10ms overhead per command.
...
|  Intercepts: "git status" -> "bdo git status"    |
+-------------------------+------------------------+
                          |
                          v
+--------------------------------------------------+
...
- Exit code propagation: Bushido never swallows non-zero exits
...
This is the full lifecycle of a command through Bushido, from LLM agent to filtered output.

### 3.1 Hook Installation (`bdo init`)
...
Bushido supports 7 agents, each with its own installation mode. The hook scripts are embedded in the binary and written at install time.
...
3. The hook calls `bdo rewrite "git status"` as a subprocess
4. `bdo rewrite` consults the command registry and returns `bdo git status`
5. The hook sends a response telling the agent to use the rewritten command
6. If anything fails (jq missing, bdo not found, no match), the hook exits silently -- the raw command runs unchanged
...
> **Details**: [`hooks/README.md`](../hooks/README.md) covers each agent's JSON format, the rewrite registry, compound command handling, and the `BDO_DISABLED` override.

#### Rewrite Pipeline

The rewrite pipeline is how Bushido intercepts and rewrites commands. The call chain is:
...
rewrite_cmd::run(cmd)                              [src/hooks/rewrite_cmd.rs]
  ...
  |     → None → exit(1)          (no Bushido equivalent, passthrough)
...
rewrite_command(cmd, excluded)                     [src/discover/registry.rs]
  ...
  |  - Simple "bdo ..." (no operators) → return as-is
...
rewrite_segment(seg, excluded)                     [src/discover/registry.rs]
  ...
  |
  |  Step 4 — Already Bushido → return as-is
  |
  |  Step 5 — Special cases (short-circuit before classification)
  |  head -N / --lines=N → rewrite_line_range() → "bdo read file --max-lines N"
  |  tail -N / -n N / --lines N → rewrite_line_range() → "bdo read file --tail-lines N"
  ...
  |  c. Guard: BDO_DISABLED=1 in prefix → None
  |  d. Guard: gh with --json/--jq/--template → None
  |  e. Apply rule's rewrite_prefixes: "cargo fmt" → "bdo cargo fmt"
...
classify_command(cmd)                              [src/discover/registry.rs]
  ...
  |  2. Check IGNORED_PREFIXES (bdo, mkdir, mv, ...)
  ...
  |
  v
Result: "bdo cargo fmt --all && bdo cargo test 2>&1 | tail -20"
  |
  |  Hook response
...
LLM Agent executes rewritten command
  (bash handles && and |, each bdo invocation is a separate process)
```
...
Once the rewritten command reaches Bushido:
...
Bushido has two filter systems:
...
1. Guard: check if the command is an Bushido meta-command (`gain`, `init`, etc.) -- if so, show Clap error
...
Analytics commands (`bdo gain`, `bdo cc-economics`, `bdo session`) query this database to produce dashboards and ROI reports.
...
| [`hooks/`](../src/hooks/README.md) | Hook system | Installation flow (`bdo init`), integrity verification, rewrite command, trust model |
| [`analytics/`](../src/analytics/README.md) | Token savings analytics | `bdo gain` dashboard, Claude Code economics, ccusage parsing |
...
Bushido supports the following LLM agents through hook integrations:

| Agent | Hook Type | Mechanism | Can Modify Command? |
|-------|-----------|-----------|---------------------|
| Claude Code | Shell hook | `PreToolUse` in `settings.json` | Yes (`updatedInput`) |
| GitHub Copilot (VS Code) | Rust binary | `bdo hook copilot` reads JSON | Yes (`updatedInput`) |
| GitHub Copilot CLI | Rust binary | `bdo hook copilot` reads JSON | No (deny + suggestion) |
| Cursor | Shell hook | `preToolUse` hook | Yes (`updated_input`) |
| Gemini CLI | Rust binary | `bdo hook gemini` reads JSON | Yes (`hookSpecificOutput`) |
...
|--------|--------|--------------|
| Startup time | < 10ms | `hyperfine 'bdo git status' 'git status'` |
| Memory usage | < 5MB resident | `/usr/bin/time -v bdo git status` |
| Binary size | < 5MB stripped | `ls -lh target/release/bdo` |
