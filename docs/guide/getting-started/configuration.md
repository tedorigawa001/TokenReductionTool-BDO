---
title: Configuration
description: Customize Bushido behavior via config.toml, environment variables, and per-project filters
sidebar:
  order: 4
---
...
bdo config --create   # create config file with defaults
```
...
|----------|-------------|
| `BDO_DISABLED=1` | Disable Bushido for a single command (`BDO_DISABLED=1 git status`) |
| `BDO_TEE_DIR` | Override the tee directory |
| `BDO_TELEMETRY_DISABLED=1` | Disable telemetry |
| `BDO_HOOK_AUDIT=1` | Enable hook audit logging |
| `SKIP_ENV_VALIDATION=1` | Skip env validation (useful with Next.js) |

## Tee system

When a command fails, Bushido saves the full raw output to a local file and prints the path:
...
```bash
BDO_DISABLED=1 git rebase main
```

## Telemetry

Bushido sends one anonymous ping per day (23h interval). No personal data, no file paths, no command content.
