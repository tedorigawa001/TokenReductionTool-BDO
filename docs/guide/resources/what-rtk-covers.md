---
title: What Bushido Optimizes
description: Commands and ecosystems automatically optimized by Bushido with typical token savings
sidebar:
  order: 1
---
...
Once Bushido is installed with a hook, these commands are automatically intercepted and filtered. You run them normally — the hook rewrites them transparently before execution.
...
| `cat` / `head` / `tail <file>` | 60-80% | Smart file reading via `bdo read` |
| `bdo smart <file>` | 85% | 2-line heuristic code summary (signatures only) |
...
These flags apply to all Bushido commands and can push savings even higher:
...
bdo git log --ultra-compact

# Debug: see what Bushido is doing
bdo git status -vvv
```
...
If a command isn't in the list above, Bushido runs it through passthrough — the output reaches the LLM unchanged. You can explicitly track unsupported commands:

```bash
bdo proxy make install    # runs make install, tracks usage, no filtering
```

To check which commands were missed opportunities: `bdo discover`.
