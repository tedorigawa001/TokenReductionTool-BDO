# Bushido Token Savings Audit Guide

Complete guide to analyzing your bdo token savings with temporal breakdowns and data exports.
...
bdo gain --all            # Show all breakdowns at once
...
# Combined flags
bdo gain --graph --history --quota    # Classic view with extras
bdo gain --daily --weekly --monthly   # Multiple breakdowns

# Reset all tracking data
bdo gain --reset          # prompts [y/N] before deleting
bdo gain --reset --yes    # skip prompt (CI/scripts)
```
...
**Metrics explained:**
- **Cmds**: Number of bdo commands executed
- **Input**: Estimated tokens from raw command output
- **Output**: Actual tokens after bdo filtering
...
bdo estimates tokens using `text.len() / 4` (4 characters per token average).
...
| `bdo pnpm list` | 70-90% | Compact dependencies |
| `bdo grep` | 70% | Truncate + group |
  ...
  stats:
    ...
    steps:
      ...
      - name: Generate report
        run: |
          bdo gain --weekly --format json > stats/week-$(date +%Y-%W).json
      - name: Commit stats
        run: |
          git add stats/
          git commit -m "Weekly bdo stats"
          git push
```
...
def send_rtk_stats():
    result = subprocess.run(['bdo', 'gain', '--format', 'json'],
                           capture_output=True, text=True)
    data = json.loads(result.stdout)

    message = f"""
    📊 *Bushido Token Savings Report*
...
cargo build --release
./target/release/bdo gain --format json
```
...
# Validate estimation
bdo git status > output.txt
python -c "
import tiktoken
enc = tiktoken.get_encoding('cl100k_base')
text = open('output.txt').read()
print(f'Actual tokens: {len(enc.encode(text))}')
print(f'bdo estimate: {len(text) // 4}')
"
```

## Best Practices

1. **Regular Exports**: `bdo gain --all --format json > monthly-$(date +%Y%m).json`
...
- [README.md](../README.md) - Full bdo documentation
