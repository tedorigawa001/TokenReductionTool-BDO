# Bushido Tracking API Documentation

Comprehensive documentation for Bushido's token savings tracking system.
...
Aggregation APIs (get_summary, get_all_days, etc.)
  ↓
CLI output (bdo gain) or JSON/CSV export
```
...
impl Tracker {
    ...
    pub fn record(
        ...
        rtk_cmd: &str,            // Bushido command (e.g., "bdo ls")
...
pub struct CommandRecord {
    ...
    pub rtk_cmd: String,           // Bushido command used
...
```rust
use bdo::tracking::{TimedExecution, Tracker};
...
fn main() -> anyhow::Result<()> {
    ...
    // Track execution
    timer.track("ls -la", "bdo ls", &input, &output);
...
```rust
use bdo::tracking::TimedExecution;
...
fn main() -> anyhow::Result<()> {
    ...
    timer.track_passthrough("git tag --list", "bdo git tag --list");
...
#!/usr/bin/env python3
"""
Export Bushido metrics to Grafana/Datadog/etc.
"""
import json
import subprocess
...
def get_rtk_metrics():
    """Fetch Bushido metrics as JSON."""
    result = subprocess.run(
        ["bdo", "gain", "--all", "--format", "json"],
        capture_output=True,
        text=True
    )
...
### Rust Integration (Using Bushido as Library)

```rust
// In your Cargo.toml
// [dependencies]
// bdo = { git = "https://github.com/tedorigawa001/TokenReductionTool" }

use bdo::tracking::{Tracker, TimedExecution};
use anyhow::Result;
...
CREATE TABLE commands (
    ...
    rtk_cmd TEXT NOT NULL,             -- Bushido command (e.g., "bdo ls")
...
- [COMMAND_AUDIT.md](../claudedocs/COMMAND_AUDIT.md) - List of all Bushido commands
