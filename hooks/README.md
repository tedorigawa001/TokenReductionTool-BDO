...
**Deployed hook artifacts** — the actual files installed on user machines by `bdo init`. These are shell scripts, TypeScript plugins, and rules files that run outside the Rust binary. They are **thin delegates**: parse agent-specific JSON, call `bdo rewrite` as a subprocess, format agent-specific response. Zero filtering logic lives here.
...
LLM agent integrations that intercept CLI commands and route them through Bushido for token optimization. Each hook transparently rewrites raw commands (e.g., `git status`) to their Bushido equivalents (e.g., `bdo git status`), delivering 60-90% token savings without requiring the agent or user to change their workflow.
...
Agent runs command (e.g., "cargo test --nocapture")
  ...
  -> Calls `bdo rewrite "cargo test --nocapture"`
  -> Registry matches pattern, returns "bdo cargo test --nocapture"
  -> Hook sends response in agent-specific JSON format
  -> Agent executes "bdo cargo test --nocapture" instead
  -> Filtered output reaches LLM (~90% fewer tokens)
```

All rewrite logic lives in the Rust binary (`src/discover/registry.rs`). Hook scripts are **thin delegates** that handle agent-specific JSON formats and call `bdo rewrite` for the actual decision. This ensures a single source of truth for all 70+ rewrite patterns.
...
| VS Code Copilot Chat | Rust binary (`bdo hook copilot`) | Transparent rewrite | Yes (`updatedInput`) |
| GitHub Copilot CLI | Rust binary (`bdo hook copilot`) | Deny-with-suggestion | No (agent retries) |
| Cursor | Shell hook (`preToolUse`) | Transparent rewrite | Yes (`updated_input`) |
| Gemini CLI | Rust binary (`bdo hook gemini`) | Transparent rewrite | Yes (`hookSpecificOutput`) |
...
```json
{
  "hookSpecificOutput": {
    ...
    "updatedInput": { "command": "bdo git status" }
  }
}
```
...
{
  "permission": "allow",
  "updated_input": { "command": "bdo git status" }
}
```
...
```json
{
  ...
  "permissionDecisionReason": "Token savings: use `bdo git status` instead"
}
```
...
```json
{
  "decision": "allow",
  "hookSpecificOutput": {
    "tool_input": { "command": "bdo git status" }
  }
}
```
...
```python
result = subprocess.run(["bdo", "rewrite", command], capture_output=True, text=True, timeout=2)
...
Example: `cargo fmt --all && cargo test` becomes `bdo cargo fmt --all && bdo cargo test`
...
- **Already-RTK**: `bdo git status` passes through unchanged (no `bdo bdo git`) <!-- bdo-stale-ignore: intentional wrong-form contrast -->
...
- bdo binary not found: warning to stderr, exit 0
- bdo version too old (< 0.23.0): warning to stderr, exit 0
- Invalid JSON input: pass through unchanged
- `bdo rewrite` crashes: hook exits 0 (subprocess error ignored)
...
Bushido supports AI coding assistants that developers actually use day-to-day. To add a new agent:
