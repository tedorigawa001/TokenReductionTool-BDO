...
echo "============================================"
echo "  Bushido Rewrite Hook Test Suite"
echo "============================================"
echo ""
...
test_rewrite "curl -s https://example.com" \
  ...
  "bdo curl -s https://example.com"

test_rewrite "cat package.json" \
  "cat package.json" \
  "bdo read package.json"

test_rewrite "grep -rn pattern src/" \
  "grep -rn pattern src/" \
  "bdo grep -rn pattern src/"
...
test_rewrite "env + npm run" \
  ...
  "NODE_ENV=test bdo npm run test:e2e"
...
test_rewrite "env + docker compose logs (supported, rewritten)" \
  ...
  "COMPOSE_PROJECT_NAME=test bdo docker compose logs web"
...
test_rewrite "docker compose up -d (NOT rewritten — unsupported by bdo)" \
  "docker compose up -d" \
  ""

test_rewrite "docker compose logs postgrest" \
  "docker compose logs postgrest" \
  "bdo docker compose logs postgrest"

test_rewrite "docker compose ps" \
  "docker compose ps" \
  "bdo docker compose ps"

test_rewrite "docker compose build" \
  "docker compose build" \
  "bdo docker compose build"

test_rewrite "docker compose down (NOT rewritten — unsupported by bdo)" \
  "docker compose down" \
  ""
...
test_rewrite "docker run --rm postgres" \
  ...
  "bdo docker run --rm postgres"

test_rewrite "docker exec -it db psql" \
  "docker exec -it db psql" \
  "bdo docker exec -it db psql"
...
test_rewrite "wget https://example.com/file" \
  ...
  "bdo wget https://example.com/file"

test_rewrite "gh api repos/owner/repo" \
  "gh api repos/owner/repo" \
  "bdo gh api repos/owner/repo"
...
test_rewrite "kubectl describe pod foo" \
  ...
  "bdo kubectl describe pod foo"

test_rewrite "kubectl apply -f deploy.yaml" \
  "kubectl apply -f deploy.yaml" \
  "bdo kubectl apply -f deploy.yaml"

echo ""

# ---- SECTION 3b: BDO_DISABLED and redirect fixes (#345, #346) ----
echo "--- BDO_DISABLED (#345) ---"
test_rewrite "BDO_DISABLED=1 git status (no rewrite)" \
  "BDO_DISABLED=1 git status" \
  ""

test_rewrite "BDO_DISABLED=1 cargo test (no rewrite)" \
  "BDO_DISABLED=1 cargo test" \
  ""

test_rewrite "FOO=1 BDO_DISABLED=1 git status (no rewrite)" \
  "FOO=1 BDO_DISABLED=1 git status" \
  ""
...
test_rewrite "cargo test &>/dev/null" \
  "cargo test &>/dev/null" \
  "bdo cargo test &>/dev/null"

# Note: the bash hook rewrites only the first command segment (sed-based);
# full compound rewriting (both sides of &) is handled by `bdo rewrite` (Rust).
...
echo "--- Audit logging (BDO_HOOK_AUDIT=1) ---"
...
test_audit_log() {
  ...
  local input_json
  input_json=$(jq -n --arg cmd "$input_cmd" '{"tool_name":"Bash","tool_input":{"command":$cmd}}')
  echo "$input_json" | BDO_HOOK_AUDIT=1 BDO_AUDIT_DIR="$AUDIT_TMPDIR" bash "$HOOK" 2>/dev/null || true
...
# Test no log when BDO_HOOK_AUDIT is unset
rm -f "$AUDIT_TMPDIR/hook-audit.log"
input_json=$(jq -n --arg cmd "git status" '{"tool_name":"Bash","tool_input":{"command":$cmd}}')
echo "$input_json" | BDO_AUDIT_DIR="$AUDIT_TMPDIR" bash "$HOOK" 2>/dev/null || true
TOTAL=$((TOTAL + 1))
if [ ! -f "$AUDIT_TMPDIR/hook-audit.log" ]; then
  printf "  ${GREEN}PASS${RESET} audit: no log when BDO_HOOK_AUDIT unset\n"
  PASS=$((PASS + 1))
else
  printf "  ${RED}FAIL${RESET} audit: log created when BDO_HOOK_AUDIT unset\n"
  FAIL=$((FAIL + 1))
fi
