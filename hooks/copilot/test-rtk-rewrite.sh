...
# Assert Copilot CLI: hook denies and reason contains the expected bdo command
test_deny() {
  local description="$1"
  local input_cmd="$2"
  ...
  local output
  output=$(copilot_bash_input "$input_cmd" | "$Bushido" hook 2>/dev/null) || true
...
test_vscode_rewrite() {
  ...
  local output
  output=$(vscode_bash_input "$input_cmd" | "$Bushido" hook 2>/dev/null) || true
...
test_allow() {
  ...
  local output
  output=$(echo "$input" | "$Bushido" hook 2>/dev/null) || true
...
echo "============================================"
echo "  Bushido Hook Test Suite (bdo hook)"
echo "============================================"
echo ""

# ---- SECTION 1: Copilot CLI — commands that should be denied ----
echo "--- Copilot CLI: intercepted (deny with bdo suggestion) ---"
...
test_allow "Copilot CLI: already bdo: bdo git status" \
  "$(copilot_bash_input "bdo git status")"

test_allow "Copilot CLI: already bdo: bdo cargo test" \
  "$(copilot_bash_input "bdo cargo test")"
...
test_allow "VS Code: already bdo" \
  "$(vscode_bash_input "bdo git status")"
...
TOTAL=$((TOTAL + 1))
raw_output=$(copilot_bash_input "git status" | "$Bushido" hook 2>/dev/null)
...
if echo "$reason" | grep -qE '`bdo [^`]+`'; then
  printf "  ${GREEN}PASS${RESET} Copilot CLI: reason contains backtick-quoted bdo command ${DIM}→ %s${RESET}\n" "$reason"
  PASS=$((PASS + 1))
else
...
TOTAL=$((TOTAL + 1))
vscode_output=$(vscode_bash_input "git status" | "$Bushido" hook 2>/dev/null)
...
if echo "$vscode_updated" | grep -q "^bdo "; then
  printf "  ${GREEN}PASS${RESET} VS Code: updatedInput.command starts with bdo ${DIM}→ %s${RESET}\n" "$vscode_updated"
  PASS=$((PASS + 1))
else
  printf "  ${RED}FAIL${RESET} VS Code: updatedInput.command should start with bdo: %s\n" "$vscode_updated"
  FAIL=$((FAIL + 1))
fi
