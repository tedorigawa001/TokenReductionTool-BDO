"""Hermes plugin adapter for Bushido command rewriting.

All rewrite logic lives in Bushido's Rust ``bdo rewrite`` command; this module
...
def _check_rtk():
    """Return whether the bdo binary is in PATH, warning once when missing."""
    global _rtk_available, _rtk_missing_warned

    if _rtk_available is None:
        _rtk_available = shutil.which("bdo") is not None

    if not _rtk_available and not _rtk_missing_warned:
        _warn("bdo binary not found in PATH; Hermes hook not registered")
...
def _pre_tool_call(tool_name=None, args=None, **_kwargs):
    """Rewrite mutable Hermes terminal command args when Bushido provides a change."""
    try:
...
def _warn(message):
    print(f"bdo: hermes plugin warning: {message}", file=sys.stderr)
