    ...
    fake_rtk.write_text(
        "\n".join(
            [
                ...
                "    print('bdo git status')",
                "    raise SystemExit(0)",
                "print('unexpected bdo args:', sys.argv[1:], file=sys.stderr)",
                "raise SystemExit(1)",
                "",
            ]
        )
...
class RtkRewritePluginTest(unittest.TestCase):
    ...
    def test_missing_rtk_skips_registering_pre_tool_call(self):
        ...
        self.assertEqual(
            "bdo: hermes plugin warning: bdo binary not found in PATH; Hermes hook not registered\n",
            stderr.getvalue(),
        )
    ...
    def test_check_rtk_caches_result_across_calls(self):
        ...
        which.assert_called_once_with("bdo")
    ...
    def test_rewrite_success_mutates_same_terminal_args_dict(self):
        ...
        with mock.patch.object(
            module.subprocess,
            "run",
            return_value=FakeCompletedProcess(stdout="bdo git status\n"),
        ):
            callback(tool_name="terminal", args=args)

        self.assertEqual({"command": "bdo git status"}, args)
    ...
    def test_rewrite_returncode_three_mutates_same_terminal_args_dict(self):
        ...
        with mock.patch.object(
            module.subprocess,
            "run",
            return_value=FakeCompletedProcess(returncode=3, stdout="bdo git status\n"),
        ):
            callback(tool_name="terminal", args=args)
    ...
    def test_unexpected_returncode_warns_with_stderr_details(self):
        ...
        with mock.patch.object(
            module.subprocess,
            "run",
            return_value=FakeCompletedProcess(returncode=4, stdout="bdo git status\n", stderr="bad news"),
        ):
            with mock.patch.object(module.sys, "stderr", new_callable=io.StringIO) as stderr:
                callback(tool_name="terminal", args=args)

        self.assertEqual({"command": "git status"}, args)
        self.assertEqual("bdo: hermes plugin warning: bdo rewrite failed with exit 4: bad news\n", stderr.getvalue())

    def test_rewrite_timeout_warns_and_preserves_original_command(self):
        module, callback = self.load_callback()
        args = {"command": "git status"}

        timeout = subprocess.TimeoutExpired(cmd=["bdo", "rewrite", "git status"], timeout=2)
        with mock.patch.object(module.subprocess, "run", side_effect=timeout):
            with mock.patch.object(module.sys, "stderr", new_callable=io.StringIO) as stderr:
                callback(tool_name="terminal", args=args)

        self.assertEqual({"command": "git status"}, args)
        self.assertEqual("bdo: hermes plugin warning: bdo rewrite timed out\n", stderr.getvalue())
    ...
    def test_file_not_found_preserves_original_command(self):
        ...
        self.assertEqual({"command": "git status"}, args)
        self.assertIn("bdo: hermes plugin warning:", stderr.getvalue())
    ...
    def test_unexpected_exception_prints_warning_and_keeps_command(self):
        ...
        self.assertEqual({"command": "git status"}, args)
        self.assertEqual("bdo: hermes plugin warning: boom\n", stderr.getvalue())
