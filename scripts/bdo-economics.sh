#!/usr/bin/env bash
# bdo-economics.sh
# Combine ccusage (tokens spent) with bdo (tokens saved) for economic analysis
...
echo -e "${BLUE}📊 Bushido Economic Impact Analysis${NC}"
echo "════════════════════════════════════════════════════════════════"
echo
...
# Check if bdo is available
if ! command -v bdo &> /dev/null; then
    echo -e "${RED}Error: bdo not found${NC}"
...
# Fetch bdo data
echo -e "${YELLOW}Fetching token savings data from bdo...${NC}"
if ! rtk_json=$(bdo gain --monthly --format json 2>/dev/null); then
    echo -e "${RED}Failed to fetch bdo data${NC}"
    exit 1
fi
...
# Parse bdo data for current month
...
${BLUE}Economic Analysis:${NC}
  Cost without bdo:    \$$total_without_rtk (estimated)
  Cost with bdo:       \$$ccusage_cost (actual)
  ${GREEN}Net savings:         \$$saved_cost ($savings_pct%)${NC}
  ROI:                 ${GREEN}Infinite${NC} (bdo is free)
...
${GREEN}Recommendation:${NC} Focus bdo usage on high-frequency commands (git, grep, ls)
