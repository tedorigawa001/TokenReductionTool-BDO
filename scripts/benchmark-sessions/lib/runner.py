...
async def run_benchmark(
    ...
    try:
        _print_step(1, total_steps, f"Creating {vms * 2} VMs ({vms} Bushido ON + {vms} Bushido OFF)")
        ...
        _print_step(3, total_steps, "Configuring Bushido on ON VMs")
        ...
        print(f"  Bushido configured on {len(on_vms)} VMs")
