/**
 * Test helpers for Bushido integration testing.
 */

import { vmExec, BDO_BIN } from "./vm";
...
/**
 * Test token savings: compare raw command output vs Bushido filtered output.
 */
...
export async function testRewrite(
  ...
  const { stdout } = await vmExec(`${BDO_BIN} rewrite '${escaped}'`);
