/**
 * Multipass VM management for Bushido integration testing.
 */

import { $ } from "bun";

const VM_NAME = "bushido-test";
...
/** Transfer Bushido source and build in release mode */
export async function vmBuildRtk(projectRoot: string): Promise<{
  buildTime: number;
  binarySize: number;
  version: string;
}> {
  console.log("[vm] Transferring Bushido source...");

  // Create tarball excluding heavy dirs and macOS resource forks (._*)
  await $`COPYFILE_DISABLE=1 tar czf /tmp/bushido-src.tar.gz --exclude target --exclude .git --exclude node_modules --exclude "index.html*" --exclude "._*" -C ${projectRoot} .`;
  await vmTransfer("/tmp/bushido-src.tar.gz", "/tmp/bushido-src.tar.gz");
  await vmExec(
    "mkdir -p /home/ubuntu/bushido && cd /home/ubuntu/bushido && tar xzf /tmp/bushido-src.tar.gz"
  );

  console.log("[vm] Building Bushido (release)...");
  const start = Date.now();
  const { stdout, exitCode } = await vmExec(
    "export PATH=$HOME/.cargo/bin:$PATH && cd /home/ubuntu/bushido && cargo build --release 2>&1 | tail -5"
  );
  ...
  const { stdout: sizeStr } = await vmExec(
    "stat -c%s /home/ubuntu/bushido/target/release/bdo"
  );
  const binarySize = parseInt(sizeStr.trim(), 10);

  const { stdout: version } = await vmExec(
    "/home/ubuntu/bushido/target/release/bdo --version"
  );
...
export async function vmEnsureReady(): Promise<void> {
  if (await vmExists()) {
    ...
    if (exitCode !== 0) {
      ...
      if (!ready) {
        throw new Error(
          "Cloud-init timed out. Check: multipass exec bushido-test -- cat /var/log/cloud-init-output.log"
        );
      }
    }
  } else {
...
export const BDO_BIN = "/home/ubuntu/bushido/target/release/bdo";
