#!/usr/bin/env bash
#
# Bushido Smoke Tests — Ruby (RSpec, RuboCop, Minitest, Bundle)
# Creates a minimal Rails app, exercises all Ruby Bushido filters, then cleans up.
# Usage: bash scripts/test-ruby.sh
#
# Prerequisites: bdo (installed), ruby, bundler, rails gem
...
printf "${BOLD}Bushido Smoke Tests — Ruby (RSpec, RuboCop, Minitest, Bundle)${NC}\n"
printf "Binary: %s (%s)\n" "$Bushido" "$(bdo --version)"
printf "Ruby: %s\n" "$(ruby --version)"
printf "Rails: %s\n" "$(rails --version)"
printf "Bundler: %s\n" "$(bundle --version)"
...
TMPDIR=$(mktemp -d /tmp/bdo-ruby-smoke-XXXXXX)
...
rails new bdo_smoke_app --minimal --skip-git --skip-docker --quiet 2>&1 | tail -1 || true
cd bdo_smoke_app
...
assert_output "bdo rspec (with failure)" \
    "failed" \
    bdo rspec

assert_output "bdo rspec spec/models/post_spec.rb (pass)" \
    "RSpec.*passed" \
    bdo rspec spec/models/post_spec.rb

assert_output "bdo rspec spec/models/post_fail_spec.rb (fail)" \
    "failed\|❌" \
    bdo rspec spec/models/post_fail_spec.rb

# ── 2. RuboCop ───────────────────────────────────────

section "RuboCop"

assert_output "bdo rubocop (with offenses)" \
    "offense" \
    bdo rubocop

assert_output "bdo rubocop app/ (with offenses)" \
...
assert_output "bdo rake test (with failure)" \
    "failure\|error\|FAIL" \
    bdo rake test

assert_output "bdo rake test single passing file" \
    "ok rake test\|0 failures" \
    bdo rake test TEST=test/models/post_pass_test.rb

assert_exit_nonzero "bdo rake test single failing file" \
    "failure\|FAIL" \
    bdo rake test test/models/post_fail_test.rb

# ── 4. Bundle install ────────────────────────────────

section "Bundle install"

assert_output "bdo bundle install (idempotent)" \
...
assert_exit_nonzero "bdo rspec exits non-zero on failure" \
    "failed\|failure" \
    bdo rspec spec/models/post_fail_spec.rb

assert_exit_nonzero "bdo rubocop exits non-zero on offenses" \
    "offense" \
    bdo rubocop app/models/rubocop_bait.rb

assert_exit_nonzero "bdo rake test exits non-zero on failure" \
    "failure\|FAIL" \
    bdo rake test test/models/post_fail_test.rb
...
assert_output "bundle exec rspec spec/models/post_spec.rb" \
    "passed\|example" \
    bdo bundle exec rspec spec/models/post_spec.rb

assert_output "bundle exec rubocop app/" \
    "offense" \
    bdo bundle exec rubocop app/
...
assert_output "bdo rubocop -A (autocorrect)" \
    "autocorrected\|rubocop\|ok\|offense\|inspected" \
    bdo rubocop -A app/models/rubocop_bait_ac.rb
...
assert_output "bdo rspec with pending example" \
    "pending" \
    bdo rspec spec/models/post_pending_spec.rb

# ── 9. RSpec text fallback ───────────────────────────

section "RSpec text fallback"

assert_output "bdo rspec --format documentation (text path)" \
    "valid\|example\|post" \
    bdo rspec --format documentation spec/models/post_spec.rb

# ── 10. RSpec empty suite ────────────────────────────

section "RSpec empty suite"

assert_output "bdo rspec nonexistent tag" \
    "0 examples\|No examples" \
    bdo rspec --tag nonexistent spec/models/post_spec.rb
...
rtk_len=$( (bdo rspec spec/models/post_spec.rb 2>&1 || true) | wc -c | tr -d ' ')
if [[ "$rtk_len" -lt "$raw_len" ]]; then
    PASS=$((PASS + 1))
    printf "  ${GREEN}PASS${NC}  rspec: bdo (%s bytes) < raw (%s bytes)\n" "$rtk_len" "$raw_len"
else
    FAIL=$((FAIL + 1))
    FAILURES+=("token savings: rspec")
    printf "  ${RED}FAIL${NC}  rspec: bdo (%s bytes) >= raw (%s bytes)\n" "$rtk_len" "$raw_len"
fi

# rubocop (exits non-zero on offenses, so || true)
raw_len=$( (bundle exec rubocop app/ 2>&1 || true) | wc -c | tr -d ' ')
rtk_len=$( (bdo rubocop app/ 2>&1 || true) | wc -c | tr -d ' ')
if [[ "$rtk_len" -lt "$raw_len" ]]; then
    PASS=$((PASS + 1))
    printf "  ${GREEN}PASS${NC}  rubocop: bdo (%s bytes) < raw (%s bytes)\n" "$rtk_len" "$raw_len"
else
    FAIL=$((FAIL + 1))
    FAILURES+=("token savings: rubocop")
    printf "  ${RED}FAIL${NC}  rubocop: bdo (%s bytes) >= raw (%s bytes)\n" "$rtk_len" "$raw_len"
fi

# rake test (passing file)
raw_len=$( (bundle exec rake test TEST=test/models/post_pass_test.rb 2>&1 || true) | wc -c | tr -d ' ')
rtk_len=$( (bdo rake test test/models/post_pass_test.rb 2>&1 || true) | wc -c | tr -d ' ')
if [[ "$rtk_len" -lt "$raw_len" ]]; then
    PASS=$((PASS + 1))
    printf "  ${GREEN}PASS${NC}  rake test: bdo (%s bytes) < raw (%s bytes)\n" "$rtk_len" "$raw_len"
else
    FAIL=$((FAIL + 1))
    FAILURES+=("token savings: rake test")
    printf "  ${RED}FAIL${NC}  rake test: bdo (%s bytes) >= raw (%s bytes)\n" "$rtk_len" "$raw_len"
fi

# bundle install (idempotent)
raw_len=$( (bundle install 2>&1 || true) | wc -c | tr -d ' ')
rtk_len=$( (bdo bundle install 2>&1 || true) | wc -c | tr -d ' ')
if [[ "$rtk_len" -lt "$raw_len" ]]; then
    PASS=$((PASS + 1))
    printf "  ${GREEN}PASS${NC}  bundle install: bdo (%s bytes) < raw (%s bytes)\n" "$rtk_len" "$raw_len"
else
    FAIL=$((FAIL + 1))
    FAILURES+=("token savings: bundle install")
    printf "  ${RED}FAIL${NC}  bundle install: bdo (%s bytes) >= raw (%s bytes)\n" "$rtk_len" "$raw_len"
fi
...
assert_output "bdo -v rspec (verbose)" \
    "RSpec\|passed\|Running\|example" \
    bdo -v rspec spec/models/post_spec.rb
