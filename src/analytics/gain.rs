...
pub fn run(
    ...
    if summary.total_commands == 0 {
        ...
        println!("Run some bdo commands to start tracking savings.");
        return Ok(());
    }
    ...
    if !daily && !weekly && !monthly && !all {
        ...
        match hook_check::status() {
            hook_check::HookStatus::Missing => {
                eprintln!(
                    "{}",
                    "[warn] No hook installed — run `bdo init -g` for automatic token savings"
                        .yellow()
                );
                eprintln!();
            }
            hook_check::HookStatus::Outdated => {
                eprintln!(
                    "{}",
                    "[warn] Hook outdated — run `bdo init -g` to update".yellow()
                );
                eprintln!();
            }
        ...
        // Lightweight BDO_DISABLED bypass check (best-effort, silent on failure)
...
/// Lightweight scan of recent Claude Code sessions for BDO_DISABLED= overuse.
...
fn check_rtk_disabled_bypass() -> Option<String> {
    ...
    if pct > 10.0 {
        Some(format!(
            "[warn] {} commands ({:.0}%) used BDO_DISABLED=1 unnecessarily — run `bdo discover` for details",
            bypassed, pct
        ))
    } else {
        None
