...
fn run_aws_filtered(
    ...
    let rtk_label = format!("bdo {}", cmd_label);
    ...
    let result = filter_fn(&stdout).unwrap_or_else(|| {
        eprintln!("bdo: filter warning: aws filter returned None, passing through raw output");
...
fn run_s3_ls(extra_args: &[String], verbose: u8) -> Result<i32> {
    ...
    if !output.status.success() {
        ...
        timer.track("aws s3 ls", "bdo aws s3 ls", &raw, &stderr);
        return Ok(exit_code);
    }
    ...
    timer.track("aws s3 ls", "bdo aws s3 ls", &raw, &result.text);
    Ok(0)
}
...
fn run_s3_transfer(operation: &str, extra_args: &[String], verbose: u8) -> Result<i32> {
    ...
    let rtk_label = format!("bdo aws s3 {}", operation);
