...
fn run_list(depth: usize, args: &[String], verbose: u8) -> Result<i32> {
    ...
    timer.track(
        ...
        &format!("bdo pnpm list --depth={}", depth),
        &result.stdout,
        &filtered,
    );
...
fn run_outdated(args: &[String], verbose: u8) -> Result<i32> {
    ...
    timer.track("pnpm outdated", "bdo pnpm outdated", &combined, &filtered);
...
fn run_install(args: &[String], verbose: u8) -> Result<i32> {
    ...
    timer.track("pnpm install", "bdo pnpm install", &combined, &filtered);
