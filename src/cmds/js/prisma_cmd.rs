...
fn run_generate(args: &[String], verbose: u8) -> Result<i32> {
    ...
    if !result.success() {
        ...
        timer.track("prisma generate", "bdo prisma generate", &raw, &raw);
        return Ok(result.exit_code);
    }

    let filtered = filter_prisma_generate(&raw);
    println!("{}", filtered);
    timer.track("prisma generate", "bdo prisma generate", &raw, &filtered);
...
fn run_migrate(subcommand: MigrateSubcommand, args: &[String], verbose: u8) -> Result<i32> {
    ...
    if !result.success() {
        ...
        timer.track(cmd_name, &format!("bdo {}", cmd_name), &raw, &raw);
        return Ok(result.exit_code);
    }
    ...
    println!("{}", filtered);
    timer.track(cmd_name, &format!("bdo {}", cmd_name), &raw, &filtered);
...
fn run_db_push(args: &[String], verbose: u8) -> Result<i32> {
    ...
    if !result.success() {
        ...
        timer.track("prisma db push", "bdo prisma db push", &raw, &raw);
        return Ok(result.exit_code);
    }

    let filtered = filter_db_push(&raw);
    println!("{}", filtered);
    timer.track("prisma db push", "bdo prisma db push", &raw, &filtered);
