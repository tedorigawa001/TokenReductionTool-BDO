...
fn validate_json_extension(file: &Path) -> Result<()> {
    if let Some(ext) = file.extension().and_then(|e| e.to_str()) {
        ...
        if let Some(fmt) = format_name {
            let mut msg = format!(
                "{} is not a JSON file (detected {}). Use `bdo read` for non-JSON files.",
                file.display(),
                fmt
            );
            if ext == "toml" && file.file_name().is_some_and(|n| n == "Cargo.toml") {
                msg.push_str(" Tip: use `bdo deps` for Cargo.toml.");
            }
            bail!("{}", msg);
        }
    }
...
pub fn run_stdin(max_depth: usize, schema_only: bool, verbose: u8) -> Result<()> {
    ...
    println!("{}", output);
    timer.track("cat - (stdin)", "bdo json -", &content, &output);
    Ok(())
}
...
mod tests {
    ...
    fn test_cargo_toml_suggests_deps() {
        ...
        assert!(err.to_string().contains("bdo deps"));
    }
