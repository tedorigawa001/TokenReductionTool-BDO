...
fn collect_changes(against: Option<&str>) -> Result<Vec<Change>> {
    // Use NUL-terminated (`-z`) output everywhere: git otherwise C-quotes paths
    // with spaces/non-ASCII, which would make the on-disk read for the stale
    // scan silently fail. `-z` emits raw, unquoted paths.
    match against {
        Some(base) => {
            let mut changes =
                parse_name_status_z(&git_stdout(&["diff", "--name-status", "-z", base])?);
            // `git diff` only sees tracked changes; add untracked files so stray
            // artifacts (new __pycache__/, etc.) are still caught in --against mode.
