...
pub fn run_streaming(
    ...
    // Fail-safe (ARCHITECTURE.md principle #4): once a streaming filter panics on
    // a line, its internal state is suspect — stop calling it and degrade to raw
    // passthrough so the rest of the output is never swallowed.
    let mut filter_panicked = false;
    ...
    if is_streaming {
                ...
                if is_stderr {
                    if !capped_err {
                        if raw_stderr.len() + line.len() < RAW_CAP {
                            ...
                            capped_err = true;
                            eprintln!("[bdo] warning: stderr exceeds 10 MiB — capture truncated");
                        }
                    }
                } else if !capped_out {
                    if raw_stdout.len() + line.len() < RAW_CAP {
                        ...
                        capped_out = true;
                        eprintln!("[bdo] warning: stdout exceeds 10 MiB — filter input truncated");
                    }
                }
                filter_fd_is_stderr = is_stderr;
                let out_opt = if filter_panicked {
                    Some(format!("{}\n", line))
                } else {
                    match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                        ...
                        Err(_) => {
                            ...
                            eprintln!("[bdo] warning: streaming filter panicked — passing through raw output");
                            Some(format!("{}\n", line))
                        }
                    }
                };
                if let Some(output) = out_opt {
                ...
                FilterMode::Buffered(filter_fn) => {
                    for line in BufReader::new(stdout).lines().map_while(Result::ok) {
                        if raw_stdout.len() + line.len() < RAW_CAP {
                            ...
                            capped_out = true;
                            eprintln!(
                                "[bdo] warning: output exceeds 10 MiB — filter input truncated"
                            );
                        }
                    }
                    ...
                    .unwrap_or_else(|_| {
                        eprintln!("[bdo] warning: filter panicked — passing through raw output");
                        raw_stdout.clone()
                    });
        ...
        raw_stderr = stderr_thread.join().unwrap_or_else(|e| {
            eprintln!("[bdo] warning: stderr reader thread panicked: {:?}", e);
            String::new()
        });
    }
...
pub(crate) mod tests {
    ...
    // Fail-safe (ARCHITECTURE.md principle #4): a panicking streaming filter must
    // not abort the process or swallow output. The panicking line — and every line
    // after it — degrades to raw passthrough, and the exit code is still preserved.
    #[cfg(not(windows))]
    #[test]
    fn test_run_streaming_filter_panic_falls_back_to_raw() {
        let mut cmd = Command::new("printf");
        cmd.arg("good1\nBOOM\ngood2\n");
        let filter = LineFilter::new(|l| {
            assert_ne!(l, "BOOM", "simulated filter panic");
            Some(format!("{}\n", l))
        });
        let result = run_streaming(
            &mut cmd,
        ...
        assert!(
            result.filtered.contains("good2"),
            "lines after the panic must degrade to raw, got: {}",
            result.filtered
        );
    }
