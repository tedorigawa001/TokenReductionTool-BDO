...
const TELEMETRY_URL: Option<&str> = option_env!("BDO_TELEMETRY_URL");
const TELEMETRY_TOKEN: Option<&str> = option_env!("BDO_TELEMETRY_TOKEN");
...
pub fn maybe_ping() {
    ...
    if std::env::var("BDO_TELEMETRY_DISABLED").unwrap_or_default() == "1" {
        return;
    }
...
fn detect_has_config() -> bool {
    dirs::config_dir()
        .map(|d| d.join("bdo/config.toml").exists())
        .unwrap_or(false)
}
...
fn detect_hook_type() -> String {
    ...
    let checks = [
        ...
        (home.join(".cursor/hooks/bdo-rewrite.json"), "cursor"),
    ];
...
fn count_custom_toml_filters() -> usize {
    ...
    // Project-local: .bdo/filters/*.toml
    if let Ok(cwd) = std::env::current_dir() {
        if let Ok(entries) = std::fs::read_dir(cwd.join(".bdo/filters")) {
            count += entries
                .filter_map(|e| e.ok())
...
mod tests {
    ...
    #[test]
    fn test_salt_file_path_is_in_bdo_dir() {
        let path = salt_file_path();
        assert!(path.to_string_lossy().contains("bdo"));
