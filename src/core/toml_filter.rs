...
///   1. `.bdo/filters.toml`              — project-local, committable with the repo
///   2. `~/.config/bdo/filters.toml`     — user-global, applies to all projects
...
///   - `BDO_NO_TOML=1`     — bypass TOML engine entirely
///   - `BDO_TOML_DEBUG=1`  — print which filter matched and line counts to stderr
///
...
impl TomlFilterRegistry {
    ...
    fn load() -> Self {
        ...
        // Priority 1: project-local .bdo/filters.toml (trust-gated)
        let project_filter_path = std::path::Path::new(".bdo/filters.toml");
                ...
                | crate::hooks::trust::TrustStatus::EnvOverride => {
                    if let Ok(content) = std::fs::read_to_string(project_filter_path) {
                        match Self::parse_and_compile(&content, "project") {
                            ...
                            Err(e) => eprintln!("[bdo] warning: .bdo/filters.toml: {}", e),
                        }
                    }
                }
                crate::hooks::trust::TrustStatus::Untrusted => {
                    eprintln!("[bdo] WARNING: untrusted project filters (.bdo/filters.toml)");
                    eprintln!("[bdo] Filters NOT applied. Run `bdo trust` to review and enable.");
                }
                crate::hooks::trust::TrustStatus::ContentChanged { .. } => {
                    eprintln!("[bdo] WARNING: .bdo/filters.toml changed since trusted.");
                    eprintln!("[bdo] Filters NOT applied. Run `bdo trust` to re-review.");
                }
            }
        }

        // Priority 2: user-global ~/.config/bdo/filters.toml
        if let Some(config_dir) = dirs::config_dir() {
            let global_path = config_dir.join(BDO_DATA_DIR).join(FILTERS_TOML);
            if let Ok(content) = std::fs::read_to_string(&global_path) {
                match Self::parse_and_compile(&content, "user-global") {
                    Ok(f) => filters.extend(f),
                    Err(e) => eprintln!("[bdo] warning: {}: {}", global_path.display(), e),
                }
            }
        }
        ...
        match Self::parse_and_compile(builtin, "builtin") {
            Ok(f) => filters.extend(f),
            Err(e) => eprintln!("[bdo] warning: builtin filters: {}", e),
        }
    ...
    fn parse_and_compile(content: &str, source: &str) -> Result<Vec<CompiledFilter>, String> {
        ...
        for (name, def) in file.filters {
            match compile_filter(name.clone(), def) {
                ...
                Err(e) => eprintln!("[bdo] warning: filter '{}' in {}: {}", name, source, e),
            }
        }
        Ok(compiled)
    }
...
fn compile_filter(name: String, def: TomlFilterDef) -> Result<CompiledFilter, String> {
    ...
    for cmd in RUST_HANDLED_COMMANDS {
        if match_regex.is_match(cmd) {
            eprintln!(
                "[bdo] warning: filter '{}' match_command matches '{}' which is already \
...
pub fn run_filter_tests(filter_name_opt: Option<&str>) -> VerifyResults {
    ...
    // Trust-gated: only verify project-local filters if trusted (SA-2025-Bushido-002)
    let project_path = std::path::Path::new(".bdo/filters.toml");
    if project_path.exists() {
        ...
        match trust_status {
            ...
            _ => {
                eprintln!("[bdo] WARNING: untrusted project filters skipped in verify");
            }
        }
    }
...
fn collect_test_outcomes(
    ...
    let file: TomlFilterFile = match toml::from_str(content) {
        Ok(f) => f,
        Err(e) => {
            eprintln!("[bdo] warning: TOML parse error during verify: {}", e);
            return;
        }
    };
    ...
    for (name, def) in file.filters {
        ...
        match compile_filter(name.clone(), def) {
            ...
            Err(e) => eprintln!("[bdo] warning: filter '{}' compilation error: {}", name, e),
        }
    }
    ...
    for (filter_name, tests) in file.tests {
        ...
        let compiled = match compiled_filters.get(&filter_name) {
            ...
            None => {
                eprintln!(
                    "[bdo] warning: [[tests.{}]] references unknown filter",
                    filter_name
                );
                continue;
            }
...
pub fn find_matching_filter(command: &str) -> Option<&'static CompiledFilter> {
    if std::env::var("BDO_TOML_DEBUG").is_ok() {
        eprintln!(
            "[bdo:toml] looking up filter for: {:?} ({} filters loaded)",
            command,
    ...
    if std::env::var("BDO_TOML_DEBUG").is_ok() {
        match result {
            Some(f) => eprintln!("[bdo:toml] matched filter: '{}'", f.name),
            None => eprintln!("[bdo:toml] no filter matched — passthrough"),
        }
    }
    result
}
