...
/// tracker.record("ls -la", "bdo ls", 1000, 200, 50)?;
///
...
impl Tracker {
    ...
    pub fn top_commands(&self, limit: usize) -> Result<Vec<String>> {
        ...
        let rows = stmt.query_map(params![limit as i64], |row| {
            ...
            // Extract just the command name (e.g. "bdo git status" → "git")
...
impl TimedExecution {
    ...
    /// let timer = TimedExecution::start();
    /// // ... execute command ...
    /// timer.track("cmd", "bdo cmd", "input", "output");
    /// ```
    pub fn start() -> Self {
        Self {
            start: Instant::now(),
    ...
    /// timer.track_passthrough("git tag", "bdo git tag");
    /// ```
...
mod tests {
    ...
    fn test_tracker_record_and_recent() {
        ...
        let test_cmd = format!("bdo git status test_{}", std::process::id());
    ...
    fn test_track_passthrough_no_dilution() {
        ...
        let cmd1 = format!("bdo cmd1_test_{}", pid);
        let cmd2 = format!("bdo cmd2_passthrough_test_{}", pid);
    ...
    fn test_timed_execution_records_time() {
        ...
        timer.track("test cmd", "bdo test", "raw input data", "filtered");

        // Verify via DB that record exists
        let tracker = Tracker::new().expect("Failed to create tracker");
        let recent = tracker.get_recent(5).expect("Failed to get recent");
        assert!(recent.iter().any(|r| r.rtk_cmd == "bdo test"));
    }
    ...
    fn test_timed_execution_passthrough() {
        let timer = TimedExecution::start();
        timer.track_passthrough("git tag", "bdo git tag (passthrough)");
    ...
    // 7. get_db_path respects environment variable BDO_DB_PATH
    ...
    fn test_reset_all_clears_both_tables() {
        ...
        tracker
            .record(
                "git status",
                &format!("bdo git status reset_test_{}", pid),
                100,
                20,
                50,
            )
