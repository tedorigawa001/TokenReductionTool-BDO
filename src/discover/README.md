...
1. **Rewrite commands** — Every LLM agent hook calls `bdo rewrite "git status"`. This module decides whether to rewrite it (`bdo git status`) or pass it through unchanged. This is the hot path — every command the LLM runs goes through here.

2. **Analyze history** — `bdo discover` scans past LLM sessions to find commands that *could have been* rewritten but weren't. Same classification logic, different consumer.
...
2. Short-circuit special cases — `head -20 file` → `bdo read file --max-lines 20`, `tail -n 5 file` → `bdo read file --tail-lines 5`. These can't go through generic prefix replacement because it would produce `bdo read -20 file` (wrong flag position)
...
4. Apply the rewrite — find the matching rule, replace the command prefix with `bdo <cmd>`, re-prepend the env prefix, re-append the redirect suffix

**Guards along the way:**
- `BDO_DISABLED=1` in the env prefix → skip rewrite
- `gh` with `--json`/`--jq`/`--template` → skip (structured output, bdo would corrupt it)
- `cat` with flags other than `-n` → skip (different semantics than `bdo read`)
- `cat`/`head`/`tail` with `>` or `>>` → skip (write operation, not a read)
- Command in `hooks.exclude_commands` config → skip

**Result**: `bdo cargo fmt --all && bdo cargo test 2>&1 | tail -20`. Bash handles the `&&` and `|` at execution time — each `bdo` invocation is a separate process.

## How History Analysis Works

`bdo discover` reads Claude Code JSONL session files. Each file contains `tool_use`/`tool_result` pairs for every command the LLM ran. The module:
...
- `rtk_cmd` — the Bushido command it maps to (e.g., `"bdo cargo"`)
