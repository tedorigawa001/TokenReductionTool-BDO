...
pub fn run(
            ...
            for part in parts {
                ...
                if prefix_contains_rtk_disabled(env_prefix) {
                    ...
                    match classify_command(actual_cmd) {
                        ...
                        _ => {
                            // BDO_DISABLED on unsupported/ignored command — not interesting
                        }
                    }
                    continue;
                }
                ...
                match classify_command(part) {
                    ...
                    Classification::Ignored => {
                        // Check if it starts with "bdo "
                        if part.trim().starts_with("bdo ") {
                            already_rtk += 1;
                        }
                        // Otherwise just skip
                    }
    ...
    // Build BDO_DISABLED examples sorted by frequency (top 5)
