...
pub fn prefix_contains_rtk_disabled(prefix_part: &str) -> bool {
    prefix_part.contains("BDO_DISABLED=")
}

/// Check if a command has BDO_DISABLED= prefix in its env prefix portion.
...
/// Strip BDO_DISABLED=X and other env prefixes, returns `(env_prefix, actual_command)`.
...
pub fn rewrite_command(
    ...
    let has_compound = trimmed.contains("&&")
        ...
        || trimmed.contains('|')
        || trimmed.contains(" & ");
    if !has_compound && (trimmed.starts_with("bdo ") || trimmed == "bdo") {
...
fn rewrite_line_range(cmd: &str) -> Option<String> {
    for re in [&*HEAD_N, &*HEAD_LINES] {
        if let Some(caps) = re.captures(cmd) {
            ...
            return Some(format!("bdo read {} --max-lines {}", file, n));
        }
    }
    ...
    for re in [
        ...
        if let Some(caps) = re.captures(cmd) {
            let n = caps.get(1)?.as_str();
            let file = caps.get(2)?.as_str();
            return Some(format!("bdo read {} --tail-lines {}", file, n));
        }
    }
    None
}
    ...
    patterns
        ...
        .filter_map(|pattern| {
            ...
            if trimmed.is_empty() || trimmed == "^" {
                eprintln!(
                    "bdo: warning: ignoring trivial exclude_commands pattern '{}'",
                    pattern
                );
                return None;
            }
            ...
            Some(match Regex::new(&anchored) {
                ...
                Err(e) => {
                    eprintln!(
                        "bdo: warning: invalid exclude_commands pattern '{}': {}",
                        pattern, e
                    );
...
fn rewrite_segment_inner(
    ...
    if !env_prefix.is_empty() {
        // #345: BDO_DISABLED=1 in env prefix → skip rewrite entirely
        // #508: warn on stderr so agents learn to stop overusing it
        if env_prefix.contains("BDO_DISABLED=") {
            eprintln!(
                "[bdo] BDO_DISABLED=1 detected — skipping filter for this command. \
                 Remove BDO_DISABLED=1 to restore token savings."
            );
            return None;
        }
        let rewritten =
    ...
    if let Some(parts) = parse_golangci_run_parts(cmd_part) {
        let rewritten = if parts.global_segment.is_empty() {
            format!("bdo golangci-lint {}", parts.run_segment)
        } else {
            format!(
                "bdo golangci-lint {} {}",
...
mod tests {
    ...
    fn test_classify_rtk_already() {
        assert_eq!(classify_command("bdo git status"), Classification::Ignored);
    }
    ...
    fn test_rewrite_git_dash_c_status() {
        assert_eq!(
            ...
            Some("bdo git -C /path/to/repo status".into())
        );
    }
    ...
    fn test_rewrite_git_dash_c_log() {
        assert_eq!(
            ...
            Some("bdo git -C /tmp/myrepo log --oneline -5".into())
        );
    }
    ...
    fn test_rewrite_git_dash_c_diff() {
        assert_eq!(
            ...
            Some("bdo git -C /home/user/project diff --name-only".into())
        );
    }
    ...
    fn test_rewrite_compound_three_segments() {
        assert_eq!(
            ...
            Some("bdo cargo fmt --all && bdo cargo clippy --all-targets && bdo cargo test".into())
        );
    }
    ...
    fn test_rewrite_already_rtk() {
        assert_eq!(
            rewrite_command_no_prefixes("bdo git status", &[]),
            Some("bdo git status".into())
        );
    }
    ...
    fn test_rewrite_background_single_amp() {
        assert_eq!(
            ...
            Some("bdo cargo test & bdo git status".into())
        );
    }
    ...
    fn test_rewrite_with_env_prefix() {
        assert_eq!(
            ...
            Some("GIT_SSH_COMMAND=ssh bdo git push".into())
        );
    }
    ...
    fn test_rewrite_cat_file() {
        assert_eq!(
            ...
            Some("bdo read src/main.rs".into())
        );
    }
    ...
    fn test_rewrite_rtk_disabled_curl() {
        assert_eq!(
            rewrite_command_no_prefixes("BDO_DISABLED=1 curl https://example.com", &[]),
            None
        );
    }
    ...
    fn test_rewrite_rtk_disabled_git_status() {
        assert_eq!(
            rewrite_command_no_prefixes("BDO_DISABLED=1 git status", &[]),
            None
        );
    }
    ...
    fn test_rewrite_rtk_disabled_multi_env() {
        assert_eq!(
            rewrite_command_no_prefixes("FOO=1 BDO_DISABLED=1 git status", &[]),
            None
        );
    }
    ...
    fn test_rewrite_rtk_disabled_subprocess_warns() {
        ...
        let output = std::process::Command::new(&rtk_bin)
            .args(["rewrite", "BDO_DISABLED=1 git status"])
            .output()
        ...
        assert!(
            stderr.contains("BDO_DISABLED=1 detected"),
    ...
    fn test_rewrite_non_rtk_disabled_env_still_rewrites() {
        assert_eq!(
            ...
            Some("SOME_VAR=1 bdo git status".into())
        );
    }
    ...
    fn test_rewrite_env_quoted_value_with_spaces() {
        assert_eq!(
            ...
            Some(r#"GIT_SSH_COMMAND="ssh -o StrictHostKeyChecking=no" bdo git push"#.into())
        );
    }
    ...
    fn test_rewrite_env_single_quoted_value_with_spaces() {
        assert_eq!(
            ...
            Some("EDITOR='vim -u NONE' bdo git commit".into())
        );
    }
    ...
    fn test_rewrite_env_quoted_plus_unquoted() {
        assert_eq!(
            ...
            Some(r#"FOO="bar baz" BAR=1 bdo git status"#.into())
        );
    }
    ...
    fn test_rewrite_env_escaped_quotes_in_value() {
        assert_eq!(
            ...
            Some(r#"FOO="he said \"hello\"" bdo git status"#.into())
        );
    }
    ...
    fn test_rewrite_redirect_plain_2_devnull() {
        ...
        assert_eq!(
            ...
            Some("bdo git status 2>/dev/null".into())
        );
    }
    ...
    fn test_rewrite_redirect_2_gt_amp_1_with_and() {
        assert_eq!(
            ...
            Some("bdo cargo test 2>&1 && echo done".into())
        );
    }
    ...
    fn test_rewrite_redirect_amp_gt_devnull() {
        assert_eq!(
            ...
            Some("bdo cargo test &>/dev/null".into())
        );
    }
    ...
    fn test_rewrite_redirect_double() {
        ...
        assert_eq!(
            ...
            Some("bdo git status 2>&1 >/dev/null".into())
        );
    }
    ...
    fn test_rewrite_head_numeric_flag() {
        ...
        assert_eq!(
            ...
            Some("bdo read src/main.rs --max-lines 20".into())
        );
    }
    ...
    fn test_rewrite_head_lines_long_flag() {
        assert_eq!(
            ...
            Some("bdo read src/lib.rs --max-lines 50".into())
        );
    }
    ...
    fn test_rewrite_tail_numeric_flag() {
        assert_eq!(
            ...
            Some("bdo read src/main.rs --tail-lines 20".into())
        );
    }
    ...
    fn test_rewrite_tail_n_space_flag() {
        assert_eq!(
            ...
            Some("bdo read src/lib.rs --tail-lines 12".into())
        );
    }
    ...
    fn test_rewrite_tail_lines_long_flag() {
        assert_eq!(
            ...
            Some("bdo read src/lib.rs --tail-lines 7".into())
        );
    }
    ...
    fn test_rewrite_diff() {
        assert_eq!(
            ...
            Some("bdo diff file1.txt file2.txt".into())
        );
    }
    ...
    fn test_rewrite_cargo_install() {
        assert_eq!(
            ...
            Some("bdo cargo install rtk".into())
        );
    }
    ...
    fn test_rewrite_kubectl_describe() {
        assert_eq!(
            ...
            Some("bdo kubectl describe pod mypod".into())
        );
    }
    ...
    fn test_rewrite_docker_run() {
        assert_eq!(
            ...
            Some("bdo docker run --rm ubuntu bash".into())
        );
    }
    ...
    fn test_rewrite_swift_test() {
        assert_eq!(
            ...
            Some("bdo swift test --parallel".into())
        );
    }
    ...
    fn test_rewrite_docker_compose_ps() {
        assert_eq!(
            ...
            Some("bdo docker compose ps".into())
        );
    }
    ...
    fn test_rewrite_docker_compose_logs() {
        assert_eq!(
            ...
            Some("bdo docker compose logs web".into())
        );
    }
    ...
    fn test_rewrite_docker_compose_build() {
        assert_eq!(
            ...
            Some("bdo docker compose build".into())
        );
    }
    ...
    fn test_rewrite_aws_ec2() {
        assert_eq!(
            ...
            Some("bdo aws ec2 describe-instances --region us-east-1".into())
        );
    }
    ...
    fn test_rewrite_psql() {
        assert_eq!(
            ...
            Some("bdo psql -U postgres -d mydb".into())
        );
    }
    ...
    fn test_classify_golangci_lint() {
        assert!(matches!(
            ...
            Classification::Supported {
                rtk_equivalent: "bdo golangci-lint run",
                ..
            }
        ));
    }
    ...
    fn test_rewrite_golangci_lint() {
        assert_eq!(
            ...
            Some("bdo golangci-lint run ./...".into())
        );
    }
    ...
    fn test_rewrite_golangci_lint_with_flag_before_run() {
        assert_eq!(
            ...
            Some("bdo golangci-lint -v run ./...".into())
        );
    }
    ...
    fn test_rewrite_golangci_lint_with_value_flag_before_run() {
        assert_eq!(
            ...
            Some("bdo golangci-lint --color never run ./...".into())
        );
    }
    ...
    fn test_rewrite_golangci_lint_with_inline_config_flag_before_run() {
        assert_eq!(
            ...
            Some("bdo golangci-lint --config=foo.yml run ./...".into())
        );
    }
    ...
    fn test_rewrite_env_prefixed_golangci_lint_with_value_flag_before_run() {
        assert_eq!(
            ...
            Some("FOO=1 bdo golangci-lint --color never run ./...".into())
        );
    }
    ...
    fn test_rewrite_prisma() {
        ...
        for command in commands {
            assert_eq!(
                ...
                Some("bdo prisma migrate dev".into()),
                "Failed for command: {}",
                command
            );
        }
    ...
    fn test_rewrite_prettier() {
        ...
        for command in commands {
            assert_eq!(
                ...
                Some("bdo prettier --check src/".into()),
                "Failed for command: {}",
                command
            );
        }
    ...
    fn test_rewrite_npm_with_args() {
        assert_eq!(
            ...
            Some("bdo npm run test".to_string()),
        );
        assert_eq!(
            rewrite_command_no_prefixes("npm exec vitest", &[]),
            Some("bdo vitest".to_string()),
        );
    }
    ...
    fn test_rewrite_gradlew() {
        assert_eq!(
            ...
            Some("bdo gradlew assembleDebug".into())
        );
    }
    ...
    fn test_rewrite_mvn_clean_install() {
        assert_eq!(
            ...
            Some("bdo mvn -B clean install".into())
        );
    }
    ...
    fn test_rewrite_compound_or() {
        ...
        assert_eq!(
            ...
            Some("bdo cargo test || bdo cargo build".into())
        );
    }
    ...
    fn test_rewrite_compound_semicolon() {
        assert_eq!(
            ...
            Some("bdo git status; bdo cargo test".into())
        );
    }
    ...
    fn test_rewrite_compound_four_segments() {
        assert_eq!(
            ...
            Some(
                "bdo cargo fmt --all && bdo cargo clippy && bdo cargo test && bdo git status"
                    .into()
            )
        );
    }
    ...
    fn test_rewrite_env_var_prefix() {
        assert_eq!(
            ...
            Some("GIT_SSH_COMMAND=ssh bdo git push origin main".into())
        );
    }
    ...
    fn test_rewrite_find_with_flags() {
        assert_eq!(
            ...
            Some("bdo find . -name '*.rs' -type f".into())
        );
    }
    ...
    fn test_all_rules_are_complete() {
        for rule in RULES {
            ...
            assert!(
                rule.rtk_cmd.starts_with("bdo "),
                "rtk_cmd '{}' must start with 'bdo '",
                rule.rtk_cmd
            );
            assert!(
    ...
    fn test_rewrite_compound_partial_exclude() {
        ...
        assert_eq!(
            ...
            Some("bdo git status && curl https://api.example.com".into())
        );
    }
    ...
    fn test_rewrite_gh_without_json_still_works() {
        assert_eq!(
            ...
            Some("bdo gh pr list".into())
        );
    }

    // --- #508: BDO_DISABLED detection helpers ---

    #[test]
    fn test_cmd_has_rtk_disabled_prefix() {
        assert!(cmd_has_rtk_disabled_prefix("BDO_DISABLED=1 git status"));
        assert!(cmd_has_rtk_disabled_prefix(
            "FOO=1 BDO_DISABLED=1 cargo test"
        ));
        assert!(cmd_has_rtk_disabled_prefix(
            "BDO_DISABLED=true git log --oneline"
        ));
        assert!(!cmd_has_rtk_disabled_prefix("git status"));
        assert!(!cmd_has_rtk_disabled_prefix("bdo git status"));
    ...
    fn test_strip_disabled_prefix() {
        assert_eq!(
            strip_disabled_prefix("BDO_DISABLED=1 git status"),
            ("BDO_DISABLED=1 ", "git status")
        );
        assert_eq!(
            strip_disabled_prefix("FOO=1 BDO_DISABLED=1 cargo test"),
            ("FOO=1 BDO_DISABLED=1 ", "cargo test")
        );
    ...
    fn test_rewrite_git_dash_c() {
        assert_eq!(
            ...
            Some("bdo git -C /tmp status".to_string())
        );
    }
    ...
    fn test_rewrite_git_no_pager() {
        assert_eq!(
            ...
            Some("bdo git --no-pager log -5".to_string())
        );
    }
    ...
    fn test_rewrite_wc() {
        assert_eq!(
            ...
            Some("bdo wc -l src/main.rs".into())
        );
    }
    ...
    fn test_rewrite_command_substitution_passthrough() {
        assert_eq!(
            ...
            Some("bdo git log $(git rev-parse HEAD~1)".into())
        );
    }
    ...
    fn test_shell_prefix_noglob() {
        assert_eq!(
            ...
            Some("noglob bdo git status".into())
        );
    }
    ...
    fn test_shell_prefix_command() {
        assert_eq!(
            ...
            Some("command bdo git status".into())
        );
    }
    ...
    fn test_shell_prefix_builtin_exec_nocorrect() {
        assert_eq!(
            ...
            Some("builtin bdo git status".into())
        );
        assert_eq!(
            rewrite_command_no_prefixes("exec git status", &[]),
            Some("exec bdo git status".into())
        );
        assert_eq!(
            rewrite_command_no_prefixes("nocorrect git status", &[]),
            Some("nocorrect bdo git status".into())
        );
    }
    ...
    fn test_transparent_prefix_strips_and_reprepends() {
        ...
        assert_eq!(
            ...
            Some("shadowenv exec -- bdo git status".into())
        );
    }
    ...
    fn test_transparent_prefix_with_test_runner() {
        ...
        assert_eq!(
            ...
            Some("shadowenv exec -- bdo cargo test".into())
        );
    }
    ...
    fn test_transparent_prefix_composed_with_builtin() {
        ...
        assert_eq!(
            ...
            Some("noglob shadowenv exec -- bdo git status".into())
        );
    }
    ...
    fn test_transparent_prefix_composed_with_env_prefix() {
        ...
        assert_eq!(
            ...
            Some("RAILS_ENV=test bundle exec bdo git status".into())
        );
    }
    ...
    fn test_env_prefix_composed_with_builtin() {
        assert_eq!(
            ...
            Some("sudo noglob bdo git status".into())
        );
    }
    ...
    fn test_transparent_prefix_multiple_configured() {
        ...
        assert_eq!(
            ...
            Some("direnv exec . bdo git status".into())
        );
    }
    ...
    fn test_transparent_prefix_overlapping_entries_use_longest_match() {
        ...
        assert_eq!(
            ...
            Some("docker exec app bdo git status".into())
        );
    }
    ...
    fn test_transparent_prefix_inside_compound() {
        ...
        assert_eq!(
            ...
            Some("shadowenv exec -- bdo git status && shadowenv exec -- bdo cargo test".into())
        );
    }
