...
pub fn format_text(report: &DiscoverReport, limit: usize, verbose: bool) -> String {
    ...
    if !report.unsupported.is_empty() {
        ...
        out.push_str(&"-".repeat(52));
        out.push('\n');
        out.push_str("-> github.com/tedorigawa001/TokenReductionTool/issues\n");
    }

    // BDO_DISABLED bypass warning
    if report.rtk_disabled_count > 0 {
        out.push_str(&format!(
            "\nBDO_DISABLED BYPASS -- {} commands ran without filtering\n",
            report.rtk_disabled_count
        ));
        out.push_str(&"-".repeat(72));
        out.push('\n');
        out.push_str("These commands used BDO_DISABLED=1 unnecessarily:\n");
        if !report.rtk_disabled_examples.is_empty() {
            out.push_str(&format!("  {}\n", report.rtk_disabled_examples.join(", ")));
        }
        out.push_str("-> Remove BDO_DISABLED=1 to recover token savings\n");
    }
...
fn append_agent_notes(out: &mut String, status: AgentIntegrationStatus) {
    if status.cursor_hook_installed {
        out.push_str("\nNote: Cursor sessions are tracked via `bdo gain` (discover scans Claude Code only)\n");
    }

    if status.hermes_plugin_installed {
        out.push_str("\nNote: Hermes plugin is installed; Hermes sessions are tracked via `bdo gain` (discover scans Claude Code only)\n");
    }

    if status.copilot_hook_installed {
        out.push_str("\nNote: GitHub Copilot sessions are tracked via `bdo gain` (discover scans Claude Code only)\n");
    }
}
...
mod tests {
    ...
    fn test_format_text_reports_copilot_detected() {
        ...
        assert!(
            output.contains("GitHub Copilot sessions are tracked via `bdo gain`"),
