...
Owns: `bdo init` installation flows (5 agents via `AgentTarget` enum + 3 special modes: Gemini, Codex, OpenCode), SHA-256 integrity verification, hook version checking, audit log analysis, `bdo rewrite` CLI entry point, and TOML filter trust management.
...
Boundary notes:
- `rewrite_cmd.rs` is a thin CLI bridge — it exists to serve hooks (hooks call `bdo rewrite` as a subprocess) and delegates entirely to `discover/registry`.
...
## Purpose
LLM agent integration layer that installs, validates, and executes command-rewriting hooks for AI coding assistants. Hooks intercept raw CLI commands (e.g., `git status`) and rewrite them to Bushido equivalents (e.g., `bdo git status`) so that LLM agents automatically benefit from token savings without explicit user configuration.

## Installation Modes

`bdo init` supports these installation flows:

| Mode | Command | Creates | Patches |
|------|---------|---------|----------|
| Default (global) | `bdo init -g` | Hook, SHA-256 hash, Bushido.md | settings.json, CLAUDE.md |
| Hook only | `bdo init -g --hook-only` | Hook, SHA-256 hash | settings.json |
| Claude-MD (legacy) | `bdo init --claude-md` | 134-line Bushido block | CLAUDE.md |
| Windsurf | `bdo init -g --agent windsurf` | `.windsurfrules` | -- |
...
Controls how `bdo init` modifies agent settings files:
...
All file operations use atomic writes (tempfile + rename) to prevent corruption on crash. Settings files are backed up to `.bak` before modification. All operations are idempotent -- running `bdo init` multiple times is safe.

## Permission Model

Bushido enforces a permission precedence that matches Claude Code's least-privilege default:
...
To add support for a new AI coding agent: (1) add the hook installation logic to `init.rs` following the existing agent patterns, (2) if the agent requires a custom hook protocol (like Gemini's `BeforeTool`), add a processor function in `hook_cmd.rs`, (3) add the agent's hook file path to `hook_check.rs` for validation, and (4) update `integrity.rs` with the expected hash for the new hook file. Test by running `bdo init` in a fresh environment and verifying the hook rewrites commands correctly in the target agent.
