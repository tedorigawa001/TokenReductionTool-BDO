...
pub fn run(since_days: u64, verbose: u8) -> Result<()> {
    ...
    if !log_path.exists() {
        ...
        println!("Enable audit mode: export BDO_HOOK_AUDIT=1 in your shell, then use Claude Code.");
        return Ok(());
    }
...
mod tests {
    ...
    fn test_parse_line_rewrite() {
        let line = "2026-02-16T14:30:01Z | rewrite | git status | bdo git status";
        let entry = parse_line(line).unwrap();
        assert_eq!(entry.action, "rewrite");
        assert_eq!(entry.original_cmd, "git status");
        assert_eq!(entry._rewritten_cmd, "bdo git status");
    }
    ...
    fn test_token_savings() {
        ...
        let raw_log = r#"2026-02-16T14:30:01Z | rewrite | git status | bdo git status
