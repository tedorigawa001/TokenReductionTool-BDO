...
use crate::core::constants::BDO_DATA_DIR;
...
fn check_and_warn() -> Option<()> {
    let warning = match status() {
        ...
        HookStatus::Missing => {
            "[bdo] /!\\ No hook installed — run `bdo init -g` for automatic token savings"
        }
        HookStatus::Outdated => "[bdo] /!\\ Hook outdated — run `bdo init -g` to update",
    };
...
fn warn_marker_path() -> Option<PathBuf> {
    let data_dir = dirs::data_local_dir()?.join(BDO_DATA_DIR);
