...
const OPENCODE_PLUGIN: &str = include_str!("../../hooks/opencode/bdo.ts");
...
const BDO_BLOCK_END: &str = "<!-- /bdo-instructions -->";
...
```bash
bdo cargo build         # Cargo build output
bdo cargo check         # Cargo check output
bdo cargo clippy        # Clippy warnings grouped by file (80%)
bdo tsc                 # TypeScript errors grouped by file/code (83%)
bdo lint                # ESLint/Biome violations grouped (84%)
bdo prettier --check    # Files needing format only (70%)
bdo next build          # Next.js build with route metrics (87%)
```

### Test (60-99% savings)
```bash
bdo cargo test          # Cargo test failures only (90%)
bdo go test             # Go test failures only (90%)
bdo jest                # Jest failures only (99.5%)
bdo vitest              # Vitest failures only (99.5%)
bdo playwright test     # Playwright failures only (94%)
bdo pytest              # Python test failures only (90%)
bdo rake test           # Ruby test failures only (90%)
bdo rspec               # RSpec test failures only (60%)
bdo test <cmd>          # Generic test wrapper - failures only
```

### Git (59-80% savings)
```bash
bdo git status          # Compact status
bdo git log             # Compact log (works with all git flags)
bdo git diff            # Compact diff (80%)
bdo git show            # Compact show (80%)
bdo git add             # Ultra-compact confirmations (59%)
bdo git commit          # Ultra-compact confirmations (59%)
bdo git push            # Ultra-compact confirmations
bdo git pull            # Ultra-compact confirmations
bdo git branch          # Compact branch list
bdo git fetch           # Compact fetch
bdo git stash           # Compact stash
bdo git worktree        # Compact worktree
```
...
```bash
bdo gh pr view <num>    # Compact PR view (87%)
bdo gh pr checks        # Compact PR checks (79%)
bdo gh run list         # Compact workflow runs (82%)
bdo gh issue list       # Compact issue list (80%)
bdo gh api              # Compact API responses (26%)
```

### JavaScript/TypeScript Tooling (70-90% savings)
```bash
bdo pnpm list           # Compact dependency tree (70%)
bdo pnpm outdated       # Compact outdated packages (80%)
bdo pnpm install        # Compact install output (90%)
bdo npm run <script>    # Compact npm script output
bdo npx <cmd>           # Compact npx command output
bdo prisma              # Prisma without ASCII art (88%)
```

### Files & Search (60-75% savings)
```bash
bdo ls <path>           # Tree format, compact (65%)
bdo read <file>         # Code reading with filtering (60%)
bdo grep <pattern>      # Search grouped by file (75%). Format flags (-c, -l, -L, -o, -Z) run raw.
bdo find <pattern>      # Find grouped by directory (70%)
```

### Analysis & Debug (70-90% savings)
```bash
bdo err <cmd>           # Filter errors only from any command
bdo log <file>          # Deduplicated logs with counts
bdo json <file>         # JSON structure without values
bdo deps                # Dependency overview
bdo env                 # Environment variables compact
bdo summary <cmd>       # Smart summary of command output
bdo diff                # Ultra-compact diffs
```
...
bdo docker logs <c>     # Deduplicated logs
bdo kubectl get         # Compact resource list
bdo kubectl logs        # Deduplicated pod logs
```

### Network (65-70% savings)
```bash
bdo curl <url>          # Compact HTTP responses (70%)
bdo wget <url>          # Compact download output (65%)
```
...
pub fn run(
    ...
    if codex {
        ...
        if install_opencode && !global {
            anyhow::bail!("OpenCode plugin is global-only. Use: bdo init -g --opencode");
        }

        if install_cursor && !global {
            anyhow::bail!("Cursor hooks are global-only. Use: bdo init -g --agent cursor");
        }

        if install_windsurf && !global {
            anyhow::bail!("Windsurf support is global-only. Use: bdo init -g --agent windsurf");
        }
...
fn prompt_telemetry_consent() -> Result<()> {
    ...
    if accepted {
        eprintln!("  Telemetry enabled. Disable anytime: bdo telemetry disable");
    } else {
...
pub fn uninstall(
    ...
    if claude_md_path.exists() {
        ...
        if working_content.contains(BDO_MD_REF) {
            let new_content = working_content
                .lines()
                .filter(|line| !line.trim().starts_with(BDO_MD_REF))
                .collect::<Vec<_>>()
                .join("\n");
        ...
        if working_content.contains(BDO_BLOCK_START) {
            ...
            if did_remove {
                ...
                claude_md_changed = true;
                removed.push("CLAUDE.md: removed bdo-instructions block".to_string());
            }
        }
    ...
    if agents_md_path.exists() {
        ...
        if working_content.contains(BDO_BLOCK_START) {
            ...
            if did_remove {
                working_content = cleaned;
                agents_changed = true;
                removed.push("AGENTS.md: removed bdo-instructions block".to_string());
            }
        }
...
fn generate_project_filters_template(ctx: InitContext) -> Result<()> {
    let InitContext { verbose, dry_run } = ctx;
    let rtk_dir = std::path::Path::new(".bdo");
    let path = rtk_dir.join("filters.toml");

    if path.exists() {
        if verbose > 0 {
            eprintln!(".bdo/filters.toml already exists, skipping template");
        }
        return Ok(());
    }
    ...
    if dry_run {
        println!(
            "[dry-run] would create .bdo/filters.toml template: {}",
            path.display()
        );
        return Ok(());
    }
...
fn generate_global_filters_template(ctx: InitContext) -> Result<()> {
    ...
    let rtk_dir = config_dir.join(crate::core::constants::BDO_DATA_DIR);
    let path = rtk_dir.join("filters.toml");
...
fn run_claude_md_mode(global: bool, install_opencode: bool, ctx: InitContext) -> Result<()> {
    ...
    if verbose > 0 {
        eprintln!("Writing bdo instructions to: {}", path.display());
    }

    let recovery_cmd = if global {
        "bdo init -g --claude-md"
    } else {
        "bdo init --claude-md"
    };
    ...
    if global {
        ...
        if !dry_run {
            println!("   Claude Code will now use bdo in all sessions");
        }
    } else if !dry_run {
        println!("   Claude Code will use bdo in this project");
    }
...
fn run_cline_mode(ctx: InitContext) -> Result<()> {
    ...
    if existing.contains("BDO") || existing.contains("bdo") {
        if !dry_run {
            println!("\nbdo already configured for Cline in this project.\n");
    ...
    if !dry_run {
        println!("  Cline will now use bdo commands for token savings.");
...
fn run_windsurf_mode(ctx: InitContext) -> Result<()> {
    ...
    if existing.contains("BDO") || existing.contains("bdo") {
        if !dry_run {
            println!("\nbdo already configured for Windsurf in this project.\n");
    ...
    if !dry_run {
        println!("  Cascade will now use bdo commands for token savings.");
...
fn run_kilocode_mode_at(base_dir: &Path, ctx: InitContext) -> Result<()> {
    ...
    if existing.contains("BDO") || existing.contains("bdo") {
        if !dry_run {
            println!("\nbdo already configured for Kilo Code in this project.\n");
            println!("  Rules: .kilocode/rules/bdo-rules.md (already present)");
        }
    } else {
        let new_content = if existing.trim().is_empty() {
        ...
        if dry_run {
            ...
            fs::write(&rules_path, &new_content)
                .context("Failed to write .kilocode/rules/bdo-rules.md")?;
    ...
    if dry_run {
        ...
        println!("  Kilo Code will now use bdo commands for token savings.");
        println!("  Test with: git status\n");
    }
...
fn run_antigravity_mode_at(base_dir: &Path, ctx: InitContext) -> Result<()> {
    ...
    if existing.contains("BDO") || existing.contains("bdo") {
        if !dry_run {
            println!("\nbdo already configured for Antigravity in this project.\n");
            println!("  Rules: .agents/rules/antigravity-bdo-rules.md (already present)");
        }
    } else {
        let new_content = if existing.trim().is_empty() {
        ...
        if dry_run {
            ...
            fs::write(&rules_path, &new_content)
                .context("Failed to write .agents/rules/antigravity-bdo-rules.md")?;
    ...
    if dry_run {
        ...
        println!("  Antigravity will now use bdo commands for token savings.");
        println!("  Test with: git status\n");
    }
...
const HERMES_PLUGIN_INIT: &str = include_str!("../../hooks/hermes/bdo-rewrite/__init__.py");
const HERMES_PLUGIN_YAML: &str = include_str!("../../hooks/hermes/bdo-rewrite/plugin.yaml");
...
fn run_codex_mode(global: bool, ctx: InitContext) -> Result<()> {
    let (agents_md_path, rtk_md_path) = if global {
        let codex_dir = resolve_codex_dir()?;
        (codex_dir.join(AGENTS_MD), codex_dir.join(BDO_MD))
    } else {
        (PathBuf::from(AGENTS_MD), PathBuf::from(BDO_MD))
    };
...
fn upsert_rtk_block(content: &str, block: &str) -> (String, RtkBlockUpsert) {
    let start_marker = BDO_BLOCK_START;
    let end_marker = BDO_BLOCK_END;
...
///
/// `label` is shown in user-facing messages (e.g., `"bdo instructions"`,
    ...
    match action {
        ...
        RtkBlockUpsert::Malformed => {
            ...
            if let Some((line_num, _)) = existing
                .lines()
                .enumerate()
                .find(|(_, line)| line.contains(BDO_BLOCK_START))
            {
...
fn codex_rtk_md_ref(codex_dir: &Path) -> String {
    format!("@{}", codex_dir.join(BDO_MD).display())
}
...
/// global=true  → `$PI_CODING_AGENT_DIR/extensions/bdo.ts`
/// global=false → `./.pi/extensions/bdo.ts`
...
/// Return OpenCode plugin path: ~/.config/opencode/plugins/bdo.ts
...
/// Show current bdo configuration
...
fn show_claude_config() -> Result<()> {
    ...
    if binary_hook_registered {
        ...
        #[cfg(unix)]
        {
            ...
            let is_thin_delegator = hook_content.contains("bdo rewrite");
            ...
            if !is_executable {
                ...
                println!(
                    "[warn] Hook: {} (outdated — run `bdo init -g` to upgrade to native binary)",
                    hook_path.display()
                );
            } else if is_executable && has_guards {
                println!(
                    "[warn] Hook: {} (legacy script v{} — run `bdo init -g` to upgrade)",
                    hook_path.display(),
                    hook_version
                );
            } else {
        ...
        {
            println!(
                "[warn] Hook: {} (legacy script — run `bdo init -g` to upgrade)",
                hook_path.display()
            );
        }
    } else {
    ...
    if hook_path.exists() && !binary_hook_registered {
        match integrity::verify_hook_at(&hook_path) {
            ...
            Ok(integrity::IntegrityStatus::Tampered { .. }) => {
                println!("[FAIL] Integrity: hook modified outside bdo init (run: bdo verify)");
            }
            Ok(integrity::IntegrityStatus::NoBaseline) => {
                println!("[warn] Integrity: no baseline hash (run: bdo init -g to establish)");
            }
    ...
    if local_claude_md.exists() {
        ...
        if content.contains("bdo") {
            println!("[ok] Local (./CLAUDE.md): bdo enabled");
        } else {
            println!("[--] Local (./CLAUDE.md): exists but bdo not configured");
        }
    } else {
    ...
    if let Ok(cursor_dir) = resolve_cursor_dir() {
        ...
        if cursor_binary_registered {
            ...
            #[cfg(unix)]
            {
                ...
                let _is_thin = content.contains("bdo rewrite");
                ...
                if !is_executable {
                    ...
                    println!(
                        "[warn] Cursor hook: {} (legacy script — run `bdo init -g --agent cursor` to upgrade)",
                        cursor_hook.display()
                    );
                }
            }
            ...
            #[cfg(not(unix))]
            {
                println!("[warn] Cursor hook: {} (legacy script — run `bdo init -g --agent cursor` to upgrade)", cursor_hook.display());
            }
        } else {
...
/// Gemini hook wrapper script — delegates to `bdo hook gemini`
const GEMINI_HOOK_SCRIPT: &str = r#"#!/bin/bash
exec bdo hook gemini
"#;
...
/// Entry point for `bdo init --gemini`
pub fn run_gemini(
    global: bool,
    hook_only: bool,
    patch_mode: PatchMode,
    ...
    let InitContext { dry_run, .. } = ctx;
    if !global {
        anyhow::bail!("Gemini support is global-only. Use: bdo init -g --gemini");
    }
...
const COPILOT_HOOK_JSON: &str = r#"{
  ...
  "hooks": {
    ...
    "preToolUse": [
      {
        "type": "command",
        "bash": "bdo hook copilot",
        "powershell": "bdo hook copilot",
        "cwd": ".",
        "timeoutSec": 5
      }
    ]
...
```bash
# Instead of:              Use:
git status                 bdo git status
git log -10                bdo git log -10
cargo test                 bdo cargo test
docker ps                  bdo docker ps
kubectl get pods           bdo kubectl pods
```
...
bdo gain --history    # Per-command savings history
bdo discover          # Find missed bdo opportunities
bdo proxy <cmd>       # Run raw (no filtering) but track usage
```
<!-- /bdo-instructions -->
"#;

/// Entry point for `bdo init --copilot`.
///
...
/// Entry point for `bdo init --uninstall --copilot` (project-scoped, like install).
    ...
    if instructions_path.exists() {
        ...
        if content.contains(BDO_BLOCK_START) {
            ...
            if did_remove {
                if dry_run {
                    println!(
                        "[dry-run] would remove bdo-instructions block from {}",
                ...
                removed.push(format!(
                    "{}: removed bdo-instructions block",
...
mod tests {
    ...
    fn test_init_mentions_all_top_level_commands() {
        for cmd in [
            ...
            assert!(
                BDO_INSTRUCTIONS.contains(cmd),
                "Missing {cmd} in BDO_INSTRUCTIONS"
            );
        }
    }
    ...
    fn test_init_has_version_marker() {
        assert!(
            BDO_INSTRUCTIONS.contains(BDO_BLOCK_START),
            "BDO_INSTRUCTIONS must start with BDO_BLOCK_START marker"
        );
        assert!(
            BDO_INSTRUCTIONS.contains(BDO_BLOCK_END),
            "BDO_INSTRUCTIONS must end with BDO_BLOCK_END marker"
        );
    }
    ...
    fn test_default_mode_creates_rtk_md() {
        ...
        assert_eq!(content, BDO_SLIM);
    }

    #[test]
    fn test_claude_md_mode_creates_full_injection() {
        // Just verify BDO_INSTRUCTIONS constant has the right content
    ...
    fn test_kilocode_mode_is_idempotent() {
        ...
        let path = temp.path().join(".kilocode/rules/bdo-rules.md");
    ...
    fn test_antigravity_mode_is_idempotent() {
        ...
        let path = temp.path().join(".agents/rules/antigravity-bdo-rules.md");
        let first = fs::read_to_string(&path).unwrap();
    ...
    fn test_hermes_mode_creates_plugin_files() {
        ...
        let plugin_dir = temp.path().join("plugins/bdo-rewrite");
        ...
        assert_eq!(
            ...
            include_str!("../../hooks/hermes/bdo-rewrite/__init__.py")
        );
        assert_eq!(
            fs::read_to_string(&manifest_path).unwrap(),
            include_str!("../../hooks/hermes/bdo-rewrite/plugin.yaml")
        );

        let config = fs::read_to_string(&config_path).unwrap();
        assert!(config.contains("plugins:\n"));
        assert!(config.contains("  enabled:\n"));
        assert_eq!(config.matches("bdo-rewrite").count(), 1);
    }
    ...
    fn test_hermes_mode_preserves_config_and_is_idempotent() {
        ...
        assert_eq!(first.matches("bdo-rewrite").count(), 1);
    }
    ...
    fn test_hermes_mode_patches_and_uninstalls_pyyaml_same_indent_missing_enabled_idempotently() {
        ...
        assert!(!uninstalled.contains("\n -\n"));
        assert_eq!(uninstalled.matches("bdo-rewrite").count(), 0);
    }
    ...
    fn test_uninstall_hermes_at_removes_plugin_dir_and_cleans_config() {
        ...
        assert_eq!(config.matches("bdo-rewrite").count(), 0);
    }
    ...
    fn test_hermes_config_patch_adds_missing_enabled_list() {
        ...
        assert_eq!(patched.matches("bdo-rewrite").count(), 1);
    }
    ...
    fn test_hermes_config_patch_pyyaml_indentationless_enabled_is_idempotent() {
        ...
        assert_eq!(patched_once.matches("bdo-rewrite").count(), 1);
    }
    ...
    fn test_hermes_config_patch_inline_enabled_is_idempotent() {
        let existing = "theme: dark\nplugins:\n  enabled: [existing-plugin, bdo-rewrite] # keep\n  search_path: ./plugins\nother: true\n";
    ...
    fn test_hermes_config_patch_inline_enabled_without_final_newline_is_idempotent() {
        let existing = "plugins:\n  enabled: [existing-plugin, bdo-rewrite]";
    ...
    fn test_hermes_config_unpatch_inline_enabled_preserves_unrelated_entries() {
        let existing = "theme: dark\nplugins:\n  enabled: [alpha, bdo-rewrite, beta] # keep comment\n  search_path: ./plugins\nother: true\n";
        ...
        assert_eq!(patched.matches("bdo-rewrite").count(), 0);
    }
    ...
    fn test_hermes_config_unpatch_removes_duplicate_inline_rtk_rewrite() {
        let existing = "plugins:\n  enabled: [alpha, bdo-rewrite, beta, bdo-rewrite]\n";
    ...
    fn test_local_init_unchanged() {
        ...
        fs::write(&claude_md, BDO_INSTRUCTIONS).unwrap();
        let content = fs::read_to_string(&claude_md).unwrap();
    ...
    fn test_upgrade_from_claude_md_to_hook_mode() {
        ...
        with_claude_dir_override(&tmp, |claude_dir| {
            ...
            assert!(
                claude_md_content.contains(BDO_BLOCK_START),
    ...
    fn test_uninstall_removes_rtk_instructions_block() {
        ...
        let (cleaned, did_remove) = remove_rtk_block(&content);
        assert!(did_remove);
        assert!(!cleaned.contains(BDO_BLOCK_START));
        assert!(!cleaned.contains("bdo cargo test"));
    }
    ...
    fn test_uninstall_handles_both_artifacts() {
        ...
        assert!(did_remove);
        assert!(!final_content.contains(BDO_BLOCK_START));
    ...
    fn test_uninstall_integration_claude_md_only() {
        let (cleaned, did_remove) = remove_rtk_block(BDO_INSTRUCTIONS);
    ...
    fn test_run_pi_mode_global_installs_plugin() {
        ...
        with_pi_dir_override(&tmp, |pi_dir| {
            ...
            assert!(
                content.contains("bdo rewrite"),
                "extension must delegate to bdo rewrite"
            );
        });
    }
    ...
    fn test_copilot_init_idempotent_repeats() {
        ...
        let count_start = after_first.matches(BDO_BLOCK_START).count();
        let count_end = after_first.matches(BDO_BLOCK_END).count();
        assert_eq!(
            count_start, 1,
            "BDO_BLOCK_START must appear once, got {count_start}"
        );
        assert_eq!(
            count_end, 1,
            "BDO_BLOCK_END must appear once, got {count_end}"
        );
    }
    ...
    fn test_copilot_hook_json_serves_both_vscode_and_cli_schemas() {
        ...
        assert_eq!(vscode["command"], "bdo hook copilot");
        ...
        assert_eq!(cli["bash"], "bdo hook copilot");
        assert_eq!(cli["powershell"], "bdo hook copilot");
        assert!(
    ...
    fn test_copilot_init_writes_dual_schema_to_disk() {
        ...
        let hook_path = temp
            ...
            .join(".github")
            .join("hooks")
            .join("bdo-rewrite.json");
        let v: serde_json::Value =
            serde_json::from_str(&fs::read_to_string(&hook_path).unwrap()).unwrap();

        assert_eq!(v["hooks"]["PreToolUse"][0]["command"], "bdo hook copilot");
        assert_eq!(v["version"], 1);
        assert_eq!(v["hooks"]["preToolUse"][0]["bash"], "bdo hook copilot");
    }
    ...
    fn test_copilot_install_does_not_touch_other_hooks() {
        ...
        assert_eq!(
            ...
            other_content,
            "third-party hook content must be unchanged by bdo install"
        );
    }
    ...
    fn test_copilot_uninstall_does_not_touch_other_hooks() {
        ...
        assert!(
            other_hook.exists(),
            "third-party hook file must survive bdo uninstall"
        );
        assert_eq!(
            fs::read_to_string(&other_hook).unwrap(),
            other_content,
            "third-party hook content must be unchanged by bdo uninstall"
        );
        assert!(
            !hooks_dir.join("bdo-rewrite.json").exists(),
    ...
    fn test_copilot_global_install_writes_hook() {
        ...
        let hook_path = temp.path().join("hooks").join("bdo-rewrite.json");
        assert!(hook_path.exists());
        let v: serde_json::Value =
            serde_json::from_str(&fs::read_to_string(&hook_path).unwrap()).unwrap();
        assert_eq!(v["version"], 1);
    ...
    fn test_copilot_global_uninstall_does_not_touch_other_hooks() {
        ...
        assert!(other.exists());
        assert_eq!(fs::read_to_string(&other).unwrap(), payload);
        assert!(!hooks_dir.join("bdo-rewrite.json").exists());
    }
    ...
    fn test_copilot_global_install_dry_run_writes_nothing() {
        ...
        assert!(!temp.path().join("hooks").join("bdo-rewrite.json").exists());
    }
