...
pub fn run_verify(verbose: u8) -> Result<()> {
    ...
    if !hook_path.exists() && !hash_file.exists() {
        ...
        if settings_path.exists() {
            ...
            if content.contains("bdo hook claude") {
                println!("PASS  native binary hook registered in settings.json");
                println!("      command: bdo hook claude");
    ...
    match verify_hook_at(&hook_path)? {
        ...
        IntegrityStatus::Tampered { expected, actual } => {
            ...
            eprintln!();
            eprintln!("  The hook file has been modified outside of `bdo init`.");
        ...
        IntegrityStatus::OrphanedHash => {
            ...
            eprintln!("      Run `bdo init -g` to reinstall.");
        }
    }
...
pub fn runtime_check() -> Result<()> {
    ...
    match verify_hook_at(&hook_path)? {
        ...
        IntegrityStatus::Tampered { expected, actual } => {
            eprintln!("bdo: hook integrity check FAILED");
            eprintln!(
                "  Expected hash: {}...",
        ...
        IntegrityStatus::OrphanedHash => {
            eprintln!("bdo: warning: hash file exists but hook is missing");
            eprintln!("  Run `bdo init -g` to reinstall.");
