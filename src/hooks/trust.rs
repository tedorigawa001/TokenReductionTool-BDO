...
//!
//! `.bdo/filters.toml` is loaded from CWD with highest priority. An attacker
...
//! - `BDO_TRUST_PROJECT_FILTERS=1` overrides for CI pipelines

use super::integrity;
use crate::core::constants::{BDO_DATA_DIR, TRUSTED_FILTERS_JSON};
...
fn store_path() -> Result<PathBuf> {
    ...
    Ok(data_dir.join(BDO_DATA_DIR).join(TRUSTED_FILTERS_JSON))
}
...
pub fn check_trust(filter_path: &Path) -> Result<TrustStatus> {
    ...
    if std::env::var("BDO_TRUST_PROJECT_FILTERS").as_deref() == Ok("1") {
        ...
        eprintln!(
            "[bdo] WARNING: BDO_TRUST_PROJECT_FILTERS=1 ignored (CI environment not detected)"
        );
    }
    ...
    let store = match read_store() {
        ...
        Err(e) => {
            eprintln!(
                "[bdo] WARNING: trust store unreadable ({}), treating all filters as untrusted",
                e
            );
            TrustStore::default()
        }
...
pub fn run_trust(list: bool) -> Result<()> {
    ...
    let filter_path = Path::new(".bdo/filters.toml");
    if !filter_path.exists() {
        anyhow::bail!("No .bdo/filters.toml found in current directory");
    }

    // Read ONCE to prevent TOCTOU: display + hash from same buffer
    let content_bytes = std::fs::read(filter_path).context("Failed to read .bdo/filters.toml")?;
    let content = String::from_utf8_lossy(&content_bytes);

    println!("=== .bdo/filters.toml ===");
    println!("{}", content);
    println!("=========================");
    println!();
    ...
    println!();
    println!(
        "Trusted .bdo/filters.toml (sha256:{})",
...
pub fn run_untrust() -> Result<()> {
    ...
    if removed {
        println!("Trust revoked for .bdo/filters.toml");
...
mod tests {
    ...
    fn test_env_override_with_ci() {
        ...
        #[allow(deprecated)]
        std::env::set_var("BDO_TRUST_PROJECT_FILTERS", "1");
        #[allow(deprecated)]
        std::env::set_var("CI", "true");
        let status = check_trust(&filter).unwrap();
        #[allow(deprecated)]
        std::env::remove_var("BDO_TRUST_PROJECT_FILTERS");
        #[allow(deprecated)]
