...
#[command(
    name = "bdo",
    version,
    about = "Bushido - Minimize LLM token consumption",
    long_about = "Bushido (bdo): a high-performance CLI proxy designed to filter and summarize system outputs before they reach your LLM context. Fork of rtk (Rust Token Killer)."
)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
enum Commands {
    ...
    /// Show hook rewrite audit metrics (requires BDO_HOOK_AUDIT=1)
...
fn run_fallback(parse_error: clap::Error) -> Result<i32> {
    ...
    // TOML filter lookup — bypass with BDO_NO_TOML=1
    ...
    if let Some(filter) = toml_match {
        ...
        match status {
            Ok(s) => {
                timer.track_passthrough(&raw_command, &format!("bdo fallback: {}", raw_command));
    ...
    match command {
        PnpmCommands::Typecheck { .. } => {
            ...
            if !filters.is_empty() {
                ...
                let msg = format!(
                    "[bdo] warning: --filter is not yet supported for pnpm {}, filters preceding the subcommand will be ignored",
                    cmd_name
                );
                return Some(msg);
            }
    ...
    let code = match cli.command {
        ...
        Commands::Read {
            ...
            for file in &files {
                let result = if file == Path::new("-") {
                    if stdin_seen {
                        eprintln!("bdo: warning: stdin specified more than once");
                        continue;
                    }
                    stdin_seen = true;
        ...
        Commands::Init {
            ...
            if show {
                ...
                if global {
                    anyhow::bail!("Kilo Code is project-scoped. Use: bdo init --agent kilocode");
                }
                hooks::init::run_kilocode_mode(ctx)?;
            } else if agent == Some(AgentTarget::Antigravity) {
                if global {
                    anyhow::bail!(
                        "Antigravity is project-scoped. Use: bdo init --agent antigravity"
                    );
                }
            ...
            match args[0].as_str() {
                ...
                "prisma" => {
                    ...
                    if args.len() > 1 {
                        ...
                        match args[1].as_str() {
                            ...
                            _ => {
                                ...
                                timer.track_passthrough(
                                    &format!("npx {}", args_str),
                                    &format!("bdo npx {} (passthrough)", args_str),
                                );
                        ...
                        timer.track_passthrough("npx prisma", "bdo npx prisma (passthrough)");
                        core::utils::exit_code_from_status(&status, "npx prisma")
                    }
                }
        ...
        Commands::Proxy { args } => {
            ...
            timer.track(
                ...
                &format!("bdo proxy {} {}", cmd_name, cmd_args.join(" ")),
                &full_output,
                &full_output,
            );
...
mod tests {
    ...
    fn test_rewrite_clap_multi_args() {
        ...
        for args in &cases {
            ...
            assert!(
                result.is_ok(),
                "bdo rewrite {:?} should parse (was failing before trailing_var_arg fix)",
                &args[2..]
            );
            if let Ok(cli) = result {
                match cli.command {
    ...
    fn test_pnpm_typecheck_with_filters() {
        ...
        match cli.command {
            Commands::Pnpm { filter, command } => {
                ...
                assert_eq!(filter, vec!["@app1", "@app2"]);
                assert_eq!(warning, "[bdo] warning: --filter is not yet supported for pnpm tsc, filters preceding the subcommand will be ignored")
            }
            _ => panic!("Expected Pnpm Build command"),
        }
    }
    ...
    fn test_broken_pipe_does_not_crash() {
        ...
        assert_ne!(
            code, 134,
            "bdo crashed with SIGABRT (exit 134) on broken pipe - SIGPIPE handler missing"
        );
    }
    ...
    fn test_npx_unknown_tool_passthrough() {
        // The bug (tedorigawa001/TokenReductionTool#815) was that unknown tools under `rtk npx`
