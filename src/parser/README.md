...
3. **Degradation warnings**: print `[Bushido:DEGRADED]` in verbose mode, `[Bushido:PASSTHROUGH]` on full fallback
...
```
[Bushido:DEGRADED] vitest parser: JSON parse failed at line 42, using regex fallback
[Bushido:PASSTHROUGH] playwright parser: Pattern mismatch, showing truncated output
```
